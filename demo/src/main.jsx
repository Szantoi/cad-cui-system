...
const readWorkspacePresetFile = file => {
...
const normalizeWorkspacePresetForPlayground = (preset, availableLayers = INITIAL_LAYERS) => {
...
function Playground() {
  ...
  const focusExitRef = useRef(null);
  ...
  const [leftDockWidth, setLeftDockWidth] = useState(DEFAULT_DOCK_LAYOUT.leftWidth);
  const [rightDockWidth, setRightDockWidth] = useState(DEFAULT_DOCK_LAYOUT.rightWidth);
  const [bottomDockHeight, setBottomDockHeight] = useState(DEFAULT_DOCK_LAYOUT.bottomHeight);
  const [bottomDockTab, setBottomDockTab] = useState('command');
  const [workspacePanelPreferences, setWorkspacePanelPreferences] = useState(INITIAL_WORKSPACE_PANEL_PREFERENCES);
  const [commandLineHeight, setCommandLineHeight] = useState(144);
  const [workspacePresets, setWorkspacePresets] = useState(loadWorkspacePresetCatalog);
  ...
  const [workspacePresetStatus, setWorkspacePresetStatus] = useState({ message: '', tone: 'neutral' });
  ...
  const [navigationOverlayPosition, setNavigationOverlayPosition] = useState(DEFAULT_VIEWPORT_NAVIGATION_OVERLAY.position);
  const [navigationOverlayCollapsed, setNavigationOverlayCollapsed] = useState(DEFAULT_VIEWPORT_NAVIGATION_OVERLAY.collapsed);
  const [selectionSummaryOverlayPosition, setSelectionSummaryOverlayPosition] = useState(DEFAULT_VIEWPORT_SELECTION_SUMMARY_OVERLAY.position);
  const [selectionSummaryOverlayCollapsed, setSelectionSummaryOverlayCollapsed] = useState(DEFAULT_VIEWPORT_SELECTION_SUMMARY_OVERLAY.collapsed);
  const [dynamicInputOverlayPosition, setDynamicInputOverlayPosition] = useState(DEFAULT_VIEWPORT_DYNAMIC_INPUT_OVERLAY.position);
  const [dynamicInputOverlayCollapsed, setDynamicInputOverlayCollapsed] = useState(DEFAULT_VIEWPORT_DYNAMIC_INPUT_OVERLAY.collapsed);
  const [viewCubeCollapsed, setViewCubeCollapsed] = useState(DEFAULT_VIEW_CUBE.collapsed);
  ...
  const [drafting, setDrafting] = useState(DEFAULT_DRAFTING);
  const [propertyState, setPropertyState] = useState(DEFAULT_PROPERTY_STATE);
  const [selectedBlockId, setSelectedBlockId] = useState('door-900');
  const [pendingBlock, setPendingBlock] = useState(null);
  const [insertOptions, setInsertOptions] = useState(DEFAULT_INSERT_OPTIONS);
