...
import { CadDockTabs } from './CadWorkspaceUi.jsx';
import { asArray, clamp, cx, useControllableState } from './cadUiUtils.js';
...
const DOCK_MODE_VALUES = new Set(Object.values(CAD_WORKSPACE_DOCK_MODES));
...
const normalizeMode = (value, fallback = CAD_WORKSPACE_DOCK_MODES.OPEN) => {
...
/** Host-owned open/rail/closed intent and clamped pixel size. */
export function useCadWorkspaceDock({
  ...
  const [storedMode, setStoredMode] = useControllableState(mode, initialMode, (nextMode, change, event) => onModeChange?.(normalizeMode(nextMode, initialMode), change, event));
  const [storedSize, setStoredSize] = useControllableState(size, initialSize, (nextSize, change, event) => onSizeChange?.(pixelValue(nextSize, initialSize, minimum, maximum), change, event));
  ...
  const setMode = useCallback((nextValue, event, source = 'programmatic') => {
    ...
    const change = { changed: nextMode !== currentMode, mode: nextMode, previousMode: currentMode, source };
  ...
  const setSize = useCallback((nextValue, event, source = 'programmatic', metadata = {}) => {
    ...
    const change = { changed: nextSize !== currentSize, size: nextSize, previousSize: currentSize, minSize: minimum, maxSize: maximum, ...metadata, source };
  ...
  return { mode: currentMode, size: currentSize, minSize: minimum, maxSize: maximum, setMode, setSize, open, rail, close, isOpen: currentMode === CAD_WORKSPACE_DOCK_MODES.OPEN, isRail: currentMode === CAD_WORKSPACE_DOCK_MODES.RAIL, isClosed: currentMode === CAD_WORKSPACE_DOCK_MODES.CLOSED };
}

/** An accessible Open / Rail / Hide control. Hosts may disable unused states. */
export function CadWorkspaceDockModeControl({
  ...
  const items = [
    { mode: CAD_WORKSPACE_DOCK_MODES.OPEN, label: openLabel || `Open ${resolvedLabel}`, caption: 'OPEN', symbol: '▤', disabled: disabled || openDisabled, onClick: onOpenClick },
    { mode: CAD_WORKSPACE_DOCK_MODES.RAIL, label: railLabel || `Rail ${resolvedLabel}`, caption: 'RAIL', symbol: '▥', disabled: disabled || railDisabled, onClick: onRailClick },
    { mode: CAD_WORKSPACE_DOCK_MODES.CLOSED, label: hideLabel || `Hide ${resolvedLabel}`, caption: 'HIDE', symbol: '×', disabled: disabled || hideDisabled, onClick: onHideClick }
  ];
  const selectMode = (nextMode, event, callback) => {
    callback?.(event, dock);
    if (!event.defaultPrevented) dock.setMode(nextMode, event, 'mode-control');
  };
  return <div {...props} className={cx('cad-workspace-dock-mode-control', className)} data-mode={dock.mode} role="group" aria-label={ariaLabel || `${resolvedLabel} visibility`}>
    {items.map(item => <button key={item.mode} type="button" className="cad-workspace-dock-mode-control__action" data-mode={item.mode} data-active={dock.mode === item.mode ? 'true' : 'false'} aria-label={item.label} aria-controls={resolvedControls} aria-pressed={dock.mode === item.mode} disabled={item.disabled} title={item.label} onClick={event => selectMode(item.mode, event, item.onClick)}><span aria-hidden="true">{item.symbol}</span><span>{item.caption}</span></button>)}
  </div>;
}

/** Pixel-based, pointer- and keyboard-accessible dock separator. */
export const CadWorkspaceDockResizeHandle = forwardRef(function CadWorkspaceDockResizeHandle({
  size,
  defaultSize = 280,
  minSize = 72,
  maxSize = 720,
  ...
  const dock = useCadWorkspaceDock({ size, defaultSize, minSize, maxSize, onSizeChange });
  ...
  useEffect(() => { if (!dragRef.current) latestSizeRef.current = dock.size; }, [dock.size]);
  const releasePointerCapture = useCallback(drag => {
    try { if (drag?.pointerId !== null && drag?.pointerId !== undefined && drag?.handle?.hasPointerCapture?.(drag.pointerId)) drag.handle.releasePointerCapture?.(drag.pointerId); } catch { /* The browser may already have released capture. */ }
  }, []);
  ...
  const pointerMoveHandler = useCallback(event => {
    ...
    setSizeRef.current?.(nextSize, event, 'pointer', { edge: drag.edge, orientation: drag.orientation, axis: drag.axis });
  }, []);
  const finishResize = useCallback((event, cancelled = false) => {
    ...
    onResizeEndRef.current?.(finalSize, { changed: finalSize !== drag.startSize, source: 'pointer', edge: drag.edge, orientation: drag.orientation, axis: drag.axis, cancelled: Boolean(cancelled) }, event);
  ...
  useEffect(() => () => {
    ...
    releasePointerCapture(drag);
    const finalSize = pixelValue(latestSizeRef.current, drag.startSize, drag.minSize, drag.maxSize);
    onResizeEndRef.current?.(finalSize, { changed: finalSize !== drag.startSize, source: 'pointer', edge: drag.edge, orientation: drag.orientation, axis: drag.axis, cancelled: true, reason: 'unmount' });
  }, [releasePointerCapture, removeWindowListeners]);
  useEffect(() => { if (disabled) finishResize(undefined, true); }, [disabled, finishResize]);
  const startResize = event => {
    ...
    const drag = { pointerId, handle: event.currentTarget, startCoordinate, startSize: dock.size, minSize: dock.minSize, maxSize: dock.maxSize, ...configuration };
    latestSizeRef.current = dock.size;
    dragRef.current = drag;
    try { if (pointerId !== null) event.currentTarget.setPointerCapture?.(pointerId); } catch { /* Global events below are the fallback. */ }
    setResizing(true);
    onResizeStartRef.current?.(dock.size, { source: 'pointer', edge: configuration.edge, orientation: configuration.orientation, axis: configuration.axis }, event);
    if (typeof window !== 'undefined') {
      const listeners = { pointerMove: pointerMoveHandler, pointerEnd: pointerEndHandler, pointerCancel: pointerCancelHandler };
  ...
  const resizeByKeyboard = (amount, event) => {
    ...
    latestSizeRef.current = nextSize;
    dock.setSize(nextSize, event, 'keyboard', { edge: configuration.edge, orientation: configuration.orientation, axis: configuration.axis });
  };
  ...
  const handleKeyDown = event => {
    ...
    if (event.key === configuration.growKey) { event.preventDefault(); resizeByKeyboard(step, event); return; }
    if (event.key === configuration.shrinkKey) { event.preventDefault(); resizeByKeyboard(-step, event); return; }
    if (event.key === 'PageUp') { event.preventDefault(); resizeByKeyboard(step * 3, event); return; }
    if (event.key === 'PageDown') { event.preventDefault(); resizeByKeyboard(-step * 3, event); return; }
    if (event.key === 'Home') { event.preventDefault(); resizeToKeyboardBound('min', event); return; }
    if (event.key === 'End') { event.preventDefault(); resizeToKeyboardBound('max', event); }
  };
  const resolvedControls = ariaControls || controls;
  const resolvedSeparatorLabel = ariaLabel || separatorLabel || `Resize ${label}`;
  return <div {...props} ref={ref} className={cx('cad-workspace-dock-resize-handle', className)} data-edge={configuration.edge} data-orientation={configuration.orientation} data-resizing={isResizing ? 'true' : 'false'} data-disabled={disabled ? 'true' : 'false'} role="separator" tabIndex={disabled ? -1 : 0} aria-label={resolvedSeparatorLabel} aria-controls={resolvedControls} aria-disabled={disabled || undefined} aria-orientation={configuration.orientation} aria-valuemin={dock.minSize} aria-valuemax={dock.maxSize} aria-valuenow={dock.size} aria-valuetext={`${dock.size} pixels`} onPointerDown={startResize} onPointerMove={onPointerMove} onPointerUp={event => { onPointerUp?.(event); finishResize(event, false); }} onPointerCancel={event => { onPointerCancel?.(event); finishResize(event, true); }} onLostPointerCapture={event => { onLostPointerCapture?.(event); finishResize(event, true); }} onKeyDown={handleKeyDown}>{children || <span className="cad-workspace-dock-resize-handle__grip" aria-hidden="true" />}</div>;
});

CadWorkspaceDockResizeHandle.displayName = 'CadWorkspaceDockResizeHandle';

/** Controlled/uncontrolled transient open state for a compact dock rail. */
export function useCadWorkspaceDockRail({ peekOpen, defaultPeekOpen = false, onPeekOpenChange, edge = 'left' } = {}) {
  const resolvedEdge = normalizeRailEdge(edge);
  const [storedPeekOpen, setStoredPeekOpen] = useControllableState(peekOpen, Boolean(defaultPeekOpen), (nextOpen, change, event) => onPeekOpenChange?.(Boolean(nextOpen), change, event));
  ...
  const setPeekOpen = useCallback((nextValue, event, source = 'programmatic') => {
    ...
    const change = { changed: nextOpen !== isPeekOpen, open: nextOpen, previousOpen: isPeekOpen, edge: resolvedEdge, source };
  ...
  return { edge: resolvedEdge, peekOpen: isPeekOpen, setPeekOpen, openPeek, closePeek };
}
...
/**
 * A compact dock rail that only previews while its joined pointer/focus region
 * is active. Clicks request durable host expansion; transient peeks are never
 * persisted by the component.
 */
export function CadWorkspaceDockRail({
  ...
  const rail = useCadWorkspaceDockRail({ edge: resolvedEdge, peekOpen, defaultPeekOpen, onPeekOpenChange });
  ...
  const expand = event => {
    if (disabled) return;
    const details = { edge: resolvedEdge, label: resolvedLabel, previewId, controls: controls || previewId, source: 'rail-expand' };
...
/** Layout-engine-neutral side/bottom zone with accessible panel tabs. */
export function CadWorkspaceDockZone({
  edge = 'left', panels = [], activeId, defaultActiveId, onActiveChange,
  onPanelClose, label = 'Docked panels', tabsLabel, compactTabs = false,
  renderPanel, children, id, className, tabsClassName, panelClassName,
  emptyLabel = 'No panels are available in this dock.', ...props
}) {
  const resolvedEdge = normalizeRailEdge(edge);
  ...
  return <section {...props} id={id} className={cx('cad-workspace-dock-zone', className)} data-edge={resolvedEdge} aria-label={label} role="complementary">
    {resolvedPanels.length > 0 ? <CadDockTabs items={resolvedPanels} activeId={activeId} defaultActiveId={defaultActiveId} onChange={(nextId, item, event) => onActiveChange?.(nextId, item, event)} onClose={onPanelClose ? (item, event) => onPanelClose(item, event) : undefined} label={tabsLabel || label} className={cx('cad-workspace-dock-zone__tabs', compactTabs && 'cad-workspace-dock-zone__tabs--compact', tabsClassName)} renderPanel={renderZonePanel} /> : <div className="cad-workspace-dock-zone__empty" role="status">{children || emptyLabel}</div>}
  </section>;
}
