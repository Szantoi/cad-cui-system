 ...
 */
export function CadPanelShell({ as: Component = 'section', tone = 'cyan', density = 'regular', visualStrength = 'standard', scroll = true, className, style, children, ...props }) {
...
export function CadPanelHeader({ icon: Icon, eyebrow, title, description, status, actions, compact = false, className, children }) {
  return (
...
export function CadPanelSection({ as: Component = 'section', icon: Icon, eyebrow, title, description, actions, compact = false, className, children, ...props }) {
...
export function CadSegmentTabs({ items, activeId, onChange, label, className }) {
  return <div className={cx('cad-ui-segment-tabs', className)} role="tablist" aria-label={label}>
    {items.map(({ id, label: itemLabel, icon: Icon, disabled = false }) => <button key={id} type="button" role="tab" aria-selected={activeId === id} disabled={disabled} onClick={() => onChange(id)}>
      {Icon && <Icon size={11} aria-hidden="true" />}<span>{itemLabel}</span>
    </button>)}
  </div>;
}

export function CadActionButton({ icon: Icon, tone = 'inherit', compact = false, className, children, type = 'button', ...props }) {
...
export function CadIconButton({ icon: Icon, label, tone = 'inherit', className, type = 'button', ...props }) {
...
export function CadDataRow({ as: Component = 'div', icon: Icon, title, detail, meta, status, actions, active = false, tone = 'inherit', className, children, ...props }) {
  const content = <>
...
export function CadStatGrid({ items, className, label = 'Summary data' }) {
...
export function CadPanelFooter({ className, children }) {
  return <footer className={cx('cad-ui-panel__footer', className)}>{children}</footer>;
}

export function CadEmptyState({ icon: Icon, title = 'NO DATA TO DISPLAY', children, className }) {
