...
import { CadToolButton } from './CadCommandUi';
import { CadPopover } from './CadOverlayUi';
import { groupCadWorkspaceRibbonCommands } from './CadWorkspaceRibbon';
...
const normalizeExplicitGroups = ({ groups, activeTabId, defaultGroupLabel }: CadAnyProps) => asArray(groups)
      ...
      {normalizedTabs.length > 0 && <div className="cad-workspace-ribbon__tabs" role="tablist" aria-label={tabListLabel}>
        {normalizedTabs.map(tab => {
          ...
          return <CadPopover
            ...
            content={({ close }: CadAnyProps) => <CadCompactRibbonDisclosure
              tab={tab}
              groups={tabGroups}
