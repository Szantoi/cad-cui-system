...
import { CadAngleInput, CadColorSwatch, CadCoordinateInput, CadLinetypePreview, CadLineweightPreview, CadNumericInput, CadUnitInput } from './CadCommandUi.jsx';
import { CadAnnotationScalePicker } from './CadDraftingUi.jsx';
import { CadColorPickerButton, CadLineweightPicker, CadLinetypePicker } from './CadLayoutUi.jsx';
import { asArray, cx, itemLabel, useControllableState } from './cadUiUtils.js';

export function CadFilterBar({ value, defaultValue = '', onChange, placeholder = 'Filter', label = 'Filter list', className, ...props }) {
...
export function CadPropertyField({ property, value, onValueChange, inputId, className }) {
...
export function CadPropertyRow({ property, value, onValueChange, className }) {
  const generatedId = useId();
  const field = property || {};
...
export function CadPropertySection({ id, title, properties = [], collapsible = true, open, defaultOpen = true, onOpenChange, onValueChange, className, children }) {
  const generatedId = useId();
...
export function CadPropertyGrid({ sections, properties, onValueChange, label = 'Properties', className, ...props }) {
...
export function CadLayerPicker({ layers = [], value, defaultValue, onChange, label = 'Current layer', className, disabled = false }) {
...
export function CadLayerRow({ layer, active = false, onActivate, onLayerChange, onColorClick, className }) {
  const item = layer || {};
...
export function CadLayerPanel({ layers = [], activeLayerId, onActiveLayerChange, onLayerChange, onAddLayer, onDeleteLayer, onColorClick, title = 'Layers', filter, defaultFilter = '', onFilterChange, filterable = true, className, emptyLabel = 'No layers match this filter' }) {
...
function CadTreeBranch({ node, level, selectedId, expandedIds, onSelect, onExpandedChange }) {
  const item = node || {};
...
export function CadObjectTree({ nodes = [], selectedId, defaultSelectedId = '', onSelect, expandedIds, defaultExpandedIds, onExpandedChange, label = 'CAD object tree', className, ...props }) {
...
export function CadTaskProgress({ label, value = 0, status, onCancel, className }) {
...
export function CadReferenceList({ references = [], onReload, onUnload, className, title = 'External references' }) {
