 ...
 */
export function CadDialog({ open = false, onClose, title, description, actions, tone = 'neutral', closeOnBackdrop = true, closeOnEscape = true, className, children, ...props }: CadAnyProps) {
  const generatedId = useId();
...
export function CadConfirmDialog({ open, title = 'Confirm action', description, confirmLabel = 'Confirm', cancelLabel = 'Cancel', destructive = false, onConfirm, onCancel, children, className, ...props }: CadAnyProps) {
...
export function CadToast({ toast, onDismiss, className }: CadAnyProps) {
  const item = toast || {};
...
export function CadToastStack({ toasts = [], onDismiss, placement = 'bottom-right', label = 'Notifications', className, ...props }: CadAnyProps) {
...
export function CadPopover({
  ...
  const triggerElement = isValidElement<CadAnyProps>(trigger)
...
export function CadTooltip({ content, placement = 'top', className, children }: CadAnyProps) {
  const generatedId = useId();
  const [visible, setVisible] = useState(false);
  if (!content || !isValidElement<CadAnyProps>(children)) return children || null;
...
export function CadShortcutReference({ shortcuts = [], title = 'Keyboard shortcuts', onClose, className, ...props }: CadAnyProps) {
  const grouped = asArray<CadAnyProps>(shortcuts).reduce<Record<string, CadAnyProps[]>>((groups, shortcut, index) => {
...
export function CadCommandPrompt({ open = true, label = 'Command input', prompt, value, defaultValue = '', onChange, onSubmit, onCancel, placeholder, submitLabel = 'Accept', className, ...props }: CadAnyProps) {
  const generatedId = useId();
