...
import { CadColorSwatch, CadLineweightPreview, CadLinetypePreview, CadShortcutHint } from './CadCommandUi';
 ...
 */
export function CadSplitPane({ orientation = 'horizontal', size, defaultSize = 30, minSize = 12, maxSize = 88, keyboardStep = 5, primary, secondary, onSizeChange, onResizeStart, onResizeEnd, separatorLabel = 'Resize panels', className, ...props }: CadAnyProps) {
...
function CadMenuBarMenu({ item, open, onToggle, onAction, onClose }: CadAnyProps) {
...
export function CadSubmenu({ item, onAction, onClose, className }: CadAnyProps) {
  const entries = normalizeItems(item?.items);
...
export function CadMenuBar({ items = [], openId, defaultOpenId = '', onOpenChange, onAction, label = 'CAD application menu', endSlot, endSlotLabel = 'Application controls', className, ...props }: CadAnyProps) {
  ...
  const focusRelative = (event, offset) => {
    const buttons = [...(event.currentTarget as HTMLElement).querySelectorAll<HTMLButtonElement>(':scope > .cad-menu-bar__menu > button:not(:disabled)')];
  ...
  const menuBar = <nav {...props} ref={rootRef} className={cx('cad-menu-bar', className)} role="menubar" aria-label={label} onKeyDown={event => {
    ...
    const activeMenuButton = document.activeElement instanceof HTMLElement ? document.activeElement : null;
    if (event.key === 'ArrowDown' && activeMenuButton?.dataset.menuId) {
      const active = normalizedItems.find(item => item.id === activeMenuButton.dataset.menuId);
...
export function CadColorPicker({ value, defaultValue = { mode: 'by-layer' }, onChange, colors = DEFAULT_COLORS, allowByLayer = true, allowByBlock = true, label = 'Color', className, ...props }: CadAnyProps) {
...
export function CadColorPickerButton({ value, onChange, label = 'Color', className, ...props }: CadAnyProps) {
  const current = normalizeColor(value);
  return <CadPopover label={label} className={cx('cad-color-picker-button', className)} trigger={<button type="button" className="cad-color-picker-button__trigger"><CadColorSwatch color={current.value || '#b4bdc7'} label={colorLabel(current)} /></button>} content={({ close }: CadAnyProps) => <CadColorPicker {...props} value={value} onChange={(nextValue, event) => { onChange?.(nextValue, event); close(event); }} label={label} />} />;
}

export function CadLinetypePicker({ linetypes = DEFAULT_LINETYPES, value, defaultValue, onChange, label = 'Linetype', className, ...props }: CadAnyProps) {
  ...
  return <CadPopover label={label} className={cx('cad-linetype-picker', className)} trigger={<button type="button" className="cad-style-picker__trigger"><CadLinetypePreview type={selected?.id || 'continuous'} label={selected?.label} /><span>⌄</span></button>} content={({ close }: CadAnyProps) => <div {...props} className="cad-style-picker" role="listbox" aria-label={label}>{options.map(option => <button key={option.id} type="button" role="option" aria-selected={option.id === selectedId} onClick={event => { setSelectedId(option.id, option, event); close(event); }}><CadLinetypePreview type={option.id} label={option.label} /></button>)}</div>} />;
}

export function CadLineweightPicker({ lineweights = DEFAULT_LINEWEIGHTS, value, defaultValue, onChange, label = 'Lineweight', className, ...props }: CadAnyProps) {
  const options = useMemo(() => normalizeItems(lineweights), [lineweights]);
  const initialValue = defaultValue ?? options[0]?.id ?? '';
  const [selectedId, setSelectedId] = useControllableState(value, initialValue, (nextValue, item, event) => onChange?.(nextValue, item, event));
  const selected = options.find(item => item.id === selectedId) || options[0];
  return <CadPopover label={label} className={cx('cad-lineweight-picker', className)} trigger={<button type="button" className="cad-style-picker__trigger"><CadLineweightPreview weight={selected?.value ?? 0.25} label={selected?.label} /><span>⌄</span></button>} content={({ close }: CadAnyProps) => <div {...props} className="cad-style-picker" role="listbox" aria-label={label}>{options.map(option => { const parsedWeight = Number(option.value ?? option.id); const weight = Number.isFinite(parsedWeight) ? parsedWeight : 0.25; return <button key={option.id} type="button" role="option" aria-selected={option.id === selectedId} onClick={event => { setSelectedId(option.id, option, event); close(event); }}><CadLineweightPreview weight={weight} label={option.label} /></button>; })}</div>} />;
}

export function CadBlockTile({ block, selected = false, onSelect, onInsert, onEdit, onDelete, renderThumbnail, className }: CadAnyProps) {
  const item = block || {};
...
export function CadBlockPalette({ blocks = [], value, defaultValue = '', onChange, onInsert, onCreate, onEdit, onDelete, filter, defaultFilter = '', onFilterChange, view = 'grid', renderThumbnail, title = 'Blocks', className, emptyLabel = 'No blocks match the current filter' }: CadAnyProps) {
...
export function CadBlockInsertOptions({ value, defaultValue = { scale: 1, rotation: 0, uniform: true, specifyOnScreen: false, explode: false }, onChange, label = 'Insert options', className }: CadAnyProps) {
