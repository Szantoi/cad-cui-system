...
var V = Object.freeze([]), H = Object.freeze({}), U = R(), W = n(null), G = 1, te = (e) => [...new Set((Array.isArray(e) ? e : V).map(M).filter(Boolean))], ne = (e) => ({
	id: M(e?.id),
	label: M(e?.label) || M(e?.id),
	detail: M(e?.detail),
	color: M(e?.color)
}), K = (e) => Object.freeze({ ...e && typeof e == "object" ? e : H }), q = (e, t) => !!(e && typeof e == "object" && Object.prototype.hasOwnProperty.call(e, t)), J = (e) => e ?? "", Y = (e) => z(e), re = (e) => Object.freeze({
	surface: M(e?.surface),
	tab: M(e?.tab),
	menu: M(e?.menu),
	group: M(e?.group),
...
function ge(e = H) {
	let t = (Array.isArray(e.commands) ? e.commands : V).map((e) => ({
		...
		requires: te(e?.requires),
		customizable: e?.customizable !== !1,
		alwaysVisible: !!e?.alwaysVisible,
		disabled: !!e?.disabled,
		active: !!e?.active,
		badge: J(e?.badge),
		selection: Y(e?.selection),
		intent: K(e?.intent),
		placements: (Array.isArray(e?.placements) ? e.placements : V).map(re)
	})).filter((e) => e.id && e.label), n = new Set(t.map((e) => e.id)), r = (Array.isArray(e.tabs) ? e.tabs : V).map((e) => ({
		id: M(e?.id),
		label: M(e?.label) || M(e?.id),
		color: M(e?.color) || "#00fbfb",
		tone: M(e?.tone) || "cyan"
	})).filter((e) => e.id), i = /* @__PURE__ */ new Set(), a = (Array.isArray(e.groups) ? e.groups : V).map(ie).filter((e) => !e.id || i.has(e.id) ? !1 : (i.add(e.id), !0)), o = e.calibration && typeof e.calibration == "object" ? e.calibration : H, s = (Array.isArray(o.accentModes) ? o.accentModes : V).map(ne).filter((e) => e.id), c = (Array.isArray(o.densities) ? o.densities : V).map(ne).filter((e) => e.id), l = (Array.isArray(o.details) ? o.details : V).map(ne).filter((e) => e.id), u = (Array.isArray(e.panels) ? e.panels : V).map((e) => ({
		...e,
		id: M(e?.id),
		...
		hiddenCommandIds: te(d.hiddenCommandIds).filter((e) => n.has(e)),
		...
		quickAccessIds: te(d.quickAccessIds).filter((e) => n.has(e)),
		recentCommandIds: V,
		commandStatus: {
			phase: "idle",
			id: "",
...
function xe(e, { state: t = H, capabilities: n = H, commandStates: r = H, selection: i = U, placement: a = e?.placement, surface: o = a?.surface || "", unavailablePresentation: s = "hide" } = H) {
	if (!e) return null;
	let c = R(i), l = B(e.selection, c), u = be(r, e, {
		command: e,
		selection: c,
		state: t,
		capabilities: n,
	...
	let h = q(u, "active") ? !!u.active : !!e.active, g = q(u, "badge") ? J(u.badge) : q(a, "badge") && a.badge !== "" ? a.badge : e.badge;
	return {
		...e,
		placement: a,
		visible: p,
...
function we(e, t) {
	let n = t && typeof t == "object" ? t : H, r = ve(e), i = Array.isArray(n.hiddenToolIds) ? e.commands.filter((e) => n.hiddenToolIds.includes(e.toolId)).map((e) => e.id) : V, a = te(n.hiddenCommandIds || i).filter((e) => r.has(e) && !r.get(e).alwaysVisible);
	return {
		version: e.version,
		...
		quickAccessIds: te(n.quickAccessIds || e.defaultState.quickAccessIds).filter((e) => r.has(e)),
		recentCommandIds: te(n.recentCommandIds).filter((e) => r.has(e)).slice(0, 8),
		commandStatus: {
			phase: "idle",
			id: "",
			error: ""
...
function De(e, t, { surface: n = "palette", tabId: r = "", menuId: i = "", groupId: a = "", capabilities: o = H, commandStates: s = H, selection: c = U, unavailablePresentation: l = "hide" } = H) {
	let u = new Set(t?.hiddenCommandIds || V), d = R(c);
...
var ke = (e, t) => e.order - t.order || e.label.localeCompare(t.label, "hu") || e.id.localeCompare(t.id, "hu"), Ae = (e, t) => {
	let n = M(e?.parentId);
	if (!n || n === e.id || !t.has(n)) return "";
	let r = /* @__PURE__ */ new Set([e.id]), i = n;
	for (; i;) {
		if (r.has(i)) return "";
		r.add(i);
		let e = t.get(i);
...
function je(e, t, { surface: n = "radial", menuId: r = "", capabilities: i = H, commandStates: a = H, selection: o = U, unavailablePresentation: s = "hide" } = H) {
	let c = (Array.isArray(e?.groups) ? e.groups : V).filter((e) => (!e.surface || e.surface === n) && (!r || !e.menu || e.menu === r)).slice().sort(ke), l = De(e, t, {
		surface: n,
		menuId: r,
		capabilities: i,
		commandStates: a,
		selection: o,
		unavailablePresentation: s
	}), u = new Map(c.map((e) => [e.id, e])), d = /* @__PURE__ */ new Map(), f = /* @__PURE__ */ new Set();
	c.forEach((e) => d.set(e.id, [])), l.forEach((e) => {
		let t = M(e?.placement?.groupId), n = u.get(t);
		if (!n) return;
		let r = e.placement.control || !n.control ? e : {
			...e,
			placement: {
				...e.placement,
				control: n.control
		...
		d.get(t)?.push(r), f.add(e.id);
	});
	let p = /* @__PURE__ */ new Map();
	c.forEach((e) => {
		p.set(e.id, {
			...e,
			commands: d.get(e.id) || V,
			children: []
		});
	});
	let m = [];
	c.forEach((e) => {
		let t = p.get(e.id), n = p.get(Ae(e, u));
		n ? n.children.push(t) : m.push(t);
	});
	let h = (e) => e.map((e) => {
		let t = h(e.children);
		return t.length === e.children.length ? e : {
			...e,
			children: t
		};
	}).filter((e) => e.commands.length || e.children.length), g = h(m), _ = l.filter((e) => !f.has(e.id));
	if (!_.length) return g;
	let v = new Set(c.map((e) => e.id)), y = "__ungrouped__", b = 2;
	for (; v.has(y);) y = `__ungrouped__-${b}`, b += 1;
	return [...g, {
		id: y,
		label: "EGYÉB PARANCSOK",
		detail: "",
...
var Me = (e) => (t, n) => {
	switch (n.type) {
		...
		case "command.completed": return {
			...t,
			recentCommandIds: te([n.commandId, ...t.recentCommandIds]).slice(0, 8),
			commandStatus: {
				phase: "idle",
				id: n.commandId,
				error: ""
...
function Ne({ registry: e = _e, capabilities: t = H, selection: n = U, commandStates: r = H, handlers: i = H, onCommand: a, shortcutScope: s, children: c }) {
	let u = y(), p = v(), [m, h] = f(Me(e), e, (e) => Te(e)), _ = d(() => ve(e), [e]), b = d(() => R(n), [n]);
	l(() => {
		Ee(e, m);
	}, [e, m]);
	let x = o((e, n) => xe(e, {
...
function Ie({ surface: e = "selection-toolbar", tabId: t = "", menuId: n = "", groupId: r = "" } = H) {
	let { selection: i, selectCommands: a, executeCommand: s } = Pe();
	return {
		selection: i,
		actions: d(() => a({
			surface: e,
...
var Le = (e, t) => e?.[t] || null;
function Re({ command: e, iconMap: t, source: n, role: r, badge: i, className: a }) {
	let { executeCommand: o } = Pe(), s = Le(t, e.icon), c = e.placement?.control || "button", l = [
		"toggle",
		"switch",
		"checkbox",
		"radio"
...
function ze({ iconMap: e = H, className: t, title: n = "PARANCS SZALAG", description: r = "Deklaratív CUI-regiszterből épített munkatéri parancsok", renderBadge: i, ...a }) {
	let { registry: o, state: s, selectCommands: c, selectCommandGroups: l, setActiveTab: u } = Pe(), d = o.tabs.find((e) => e.id === s.activeTab) || o.tabs[0], f = c({
		surface: "ribbon",
		tabId: d?.id
	}), p = o.groups?.length ? l({
		surface: "ribbon",
			...
			children: [/* @__PURE__ */ g(E, {
				...
				children: p.map((t) => /* @__PURE__ */ _("section", {
					...
					children: [/* @__PURE__ */ _("header", { children: [t.label, t.detail && /* @__PURE__ */ g("small", { children: t.detail })] }), /* @__PURE__ */ g("div", {
						...
						children: t.commands.map((t) => /* @__PURE__ */ g(Re, {
							command: t,
							iconMap: e,
							source: "ribbon",
							badge: i?.(t) ?? t.label
				...
				children: f.map((t) => /* @__PURE__ */ g(Re, {
					command: t,
					iconMap: e,
					source: "ribbon",
					badge: i?.(t) ?? t.label
...
function Be({ iconMap: e = H, commandIds: t, className: n, ...r }) {
	let { registry: i, state: a, resolveCommand: o } = Pe(), s = (Array.isArray(t) ? t : a.quickAccessIds).map((e) => i.commands.find((t) => t.id === e)).filter(Boolean).map((e) => {
	...
	return /* @__PURE__ */ g("div", {
		...
		children: s.map((t) => /* @__PURE__ */ g(Re, {
			command: t,
			iconMap: e,
			source: "quick-access"
		}, t.id))
...
function Ve({ menuId: e = "canvas", iconMap: t = H, className: n, onClose: r, ...i }) {
	let { selectCommands: a } = Pe(), o = a({
		surface: "context",
		menuId: e
	});
	return /* @__PURE__ */ _(C, {
		...
		children: [/* @__PURE__ */ g(w, {
			...
			children: /* @__PURE__ */ _("div", {
				className: "cad-cui-command-grid",
				children: [o.map((e) => /* @__PURE__ */ g(Re, {
					command: e,
					iconMap: t,
					source: "context",
					role: "menuitem"
...
function He({ iconMap: e = H, className: t, ...n }) {
	let { selectCommands: r, state: i } = Pe(), [a, o] = m(""), s = c(a), l = d(() => {
		...
		children: [
			...
			/* @__PURE__ */ _(T, {
				...
				children: [
					...
					/* @__PURE__ */ _("div", {
						className: "cad-cui-command-grid",
						children: [l.map((t) => /* @__PURE__ */ g(Re, {
							command: t,
							iconMap: e,
							source: "palette"
...
function Ue({ className: e, ...t }) {
	let { registry: n, state: r, setPreference: i, toggleCommandVisibility: a, resetPreferences: o } = Pe(), s = new Set(r.hiddenCommandIds);
	return /* @__PURE__ */ _(C, {
		...t,
		tone: "magenta",
		className: e,
...
var We = (e, t, n) => Number.isFinite(e) ? Number.isFinite(t) && e < t ? t : Number.isFinite(n) && e > n ? n : e : e, Ge = (e, t, n) => {
	e?.disabled || (e?.onClick?.(e, t), n?.(e, t));
};
function Ke({ shortcut: e, className: t }) {
	return e ? /* @__PURE__ */ g("kbd", {
		className: X("cad-shortcut-hint", t),
		children: e
	}) : null;
}
function qe({ icon: t, label: n, shortcut: r, active: i = !1, toggle: a = !1, tone: o = "inherit", badge: s, compact: c = !1, className: l, children: u, title: d, type: f = "button", ...p }) {
...
function Ye({ icon: e, label: t, shortcut: n, tone: r = "inherit", disabled: i = !1, menu: a, menuId: o, menuOpen: s, defaultMenuOpen: c = !1, onMenuOpenChange: d, onClick: f, className: m, children: h, ...v }) {
...
function Xe({ label: e, items: t = [], onAction: n, className: r, children: i }) {
	return /* @__PURE__ */ _("section", {
		...
		children: [/* @__PURE__ */ _("div", {
			...
			children: [Z(t).map((e, t) => {
				...
				return e?.type === "split" ? /* @__PURE__ */ g(Ye, {
					...i,
					menu: e.menu,
					menuOpen: e.menuOpen,
					onMenuOpenChange: (t, n) => e.onMenuOpenChange?.(t, e, n),
					onClick: a
				}, r) : e?.toggle ? /* @__PURE__ */ g(Je, {
					...i,
					onChange: (t, r) => {
						e.onChange?.(t, e, r), n?.({
							...e,
...
function Ze({ groups: e, items: t, label: n = "CAD tools", orientation: r = "horizontal", onAction: i, className: a, children: o, ...s }) {
	...
	return /* @__PURE__ */ _("div", {
		...
		"aria-label": n,
		"aria-orientation": r,
		children: [c.map((e, t) => /* @__PURE__ */ g(Xe, {
			label: e.label,
			items: e.items,
			onAction: i
		}, e.id || e.label || t)), o]
...
function Qe({ groups: e, items: t, label: n = "CAD tool palette", layout: r = "strip", className: i, ...a }) {
...
function $e({ id: e, label: t, value: n, defaultValue: r = "", onValueChange: i, onChange: a, min: o, max: s, step: c = 1, unit: l, prefix: d, suffix: f, asNumber: p = !0, disabled: m = !1, readOnly: h = !1, showSteppers: v = !0, className: y, inputClassName: b, ...x }) {
...
function et({ unit: e = "mm", ...t }) {
	return /* @__PURE__ */ g($e, {
		...t,
		unit: e
	});
}
function tt({ unit: e = "°", ...t }) {
	return /* @__PURE__ */ g($e, {
		...t,
		unit: e
	});
}
function nt({ value: e, defaultValue: t = {
	x: "",
	y: "",
	z: ""
...
function rt({ color: e = "#ffffff", label: t, size: n = "regular", onClick: r, className: i, style: a, ...o }) {
...
function it({ type: e = "continuous", color: t = "currentColor", label: n, className: r }) {
	return /* @__PURE__ */ _("span", {
...
function at({ weight: e = .25, color: t = "currentColor", label: n, className: r }) {
	return /* @__PURE__ */ _("span", {
...
function st({ item: t, label: n, detail: r, shortcut: i, icon: a, checked: o, disabled: s = !1, type: c = "action", tone: l = "inherit", onClick: u, className: d }) {
	...
	return /* @__PURE__ */ _("button", {
		...
		children: [
			...
			i && /* @__PURE__ */ g(Ke, { shortcut: i })
		]
	});
}
function ct({ items: e = [], label: t = "CAD menu", onAction: n, onClose: r, className: i, children: a, menuRef: o, ...s }) {
	...
	return /* @__PURE__ */ _("div", {
		...
		children: [Z(e).map((e, t) => e?.type === "separator" ? /* @__PURE__ */ g(ot, {}, e.id || `separator-${t}`) : /* @__PURE__ */ g(st, {
			item: e,
			label: Q(e),
			detail: e.detail,
			shortcut: e.shortcut,
			...
			type: e.type,
			tone: e.tone,
			onClick: (e, t) => Ge(e, t, n)
		}, e.id || `${Q(e)}-${t}`)), a]
	});
}
function lt({ open: t = !1, position: n = {
	x: 0,
	y: 0
...
var ut = (e) => Z(e).filter((e) => e && e.type !== "separator" && !e.hidden), dt = (e) => {
...
}, ft = (e, t) => String(e?.id ?? e?.key ?? (Q(e) || `item-${t}`)), pt = (e, t) => e.length === t.length && e.every((e, n) => e === t[n]), mt = (e, t) => {
	let n = ut(e), r = [];
	for (let e of Z(t)) {
		let t = String(e), i = n.findIndex((e, n) => ft(e, n) === t), a = i < 0 ? void 0 : n[i], o = dt(a);
		if (!a || !o.length) break;
		r.push(ft(a, i)), n = o;
	}
	return r;
}, ht = (e, t, n) => {
	let r = Math.floor(e / n), i = r * n, a = Math.min(n, t - i);
	return {
		angle: -90 + 360 * (e - i) / Math.max(a, 1),
		radius: r === 0 ? "var(--cad-radial-menu-radius)" : `calc(var(--cad-radial-menu-radius) + ${r} * var(--cad-radial-menu-ring-gap))`,
		ring: r
	};
}, gt = (e, t) => {
	let n = ut(e);
	return n.reduce((e, n) => Math.max(e, gt(dt(n), t)), Math.max(1, Math.ceil(n.length / t)));
};
function _t({ open: t = !1, position: n = {
	x: 0,
	y: 0
}, items: r = [], label: i = "CAD radial menu", centerLabel: a = "Actions", onAction: o, onClose: s, restoreFocusRef: c, returnFocusRef: d, className: f, style: m, children: h, menuRef: v, onKeyDown: y, submenuTrigger: b = "both", submenuDelay: x = 140, expandedPath: S, defaultExpandedPath: C = [], onExpandedPathChange: w, maxItemsPerRing: T, ringCapacity: E, presentation: D = "cascade", ...O }) {
	let ee = p(null), k = v || ee, A = `cad-radial-menu-${u()}`, j = p(!1), M = c || d, N = p(!1), P = p(null), F = p(null), I = p(null), L = e.useMemo(() => ut(r), [r]), [R, z] = $(S, C, (e, t, n) => w?.(e, t, n)), B = b === "hover" || b === "click" || b === "both" ? b : "both", V = D === "rings" ? "rings" : "cascade", H = Math.max(0, Number(x) || 0), U = Math.max(1, Math.floor(Number(T ?? E) || 6)), W = e.useMemo(() => mt(L, R), [L, R]), G = Math.max(1, Math.ceil(L.length / U)), te = gt(L, U), ne = e.useCallback(() => {
		F.current !== null && window.clearTimeout(F.current), F.current = null;
	}, []), K = e.useCallback(() => {
		I.current !== null && window.clearTimeout(I.current), I.current = null;
	}, []), q = e.useCallback((e) => {
		let t = e === void 0 ? k.current : k.current?.querySelector(`.cad-radial-menu__level[data-radial-level="${e}"]`);
		return [...t?.querySelectorAll("[role^=\"menuitem\"]") || []].filter((e) => !e.disabled && e.closest(".cad-radial-menu__level") === t);
	}, [k]), J = e.useCallback((e, t) => {
		let n = q(e);
		(t === void 0 ? n[0] : n.find((e) => e.getAttribute("data-radial-item-id") === t) || n[0])?.focus();
	}, [q]), Y = e.useCallback((e, t, n = void 0) => {
		j.current = t === "escape" || t === "outside", s?.(e, {
			reason: t,
			item: n
		});
	}, [s]), re = e.useCallback((e, t, n) => {
		z(e, t, n);
	}, [z]), ie = e.useCallback((e, t, n, { focusChild: r = !1, reason: i = "expand" } = {}) => {
		if (!(e?.disabled || !dt(e).length)) {
			if (r && (P.current = { depth: t.length }), !pt(W, t)) {
				re(t, n, {
					reason: i,
					item: e,
					path: t
			...
			r && window.setTimeout(() => J(t.length), 0);
		}
	}, [
		J,
		W,
		...
		let t = W[W.length - 1], n = W.slice(0, -1);
		return P.current = {
			depth: n.length,
			itemId: t
		}, re(n, e, {
	...
	}, [W, re]), oe = e.useCallback((e, t, n) => {
		(B === "hover" || B === "both") && (ne(), K(), F.current = window.setTimeout(() => {
			F.current = null, ie(e, t, n, { reason: "hover" });
		}, H));
	}, [
		K,
		ne,
		...
		W.length && (ne(), K(), I.current = window.setTimeout(() => {
			I.current = null, re([], e, {
				reason: "pointerleave",
				path: []
			});
		...
		let n = t instanceof Element ? t.closest(".cad-radial-menu__level") : null, r = Number(n?.getAttribute("data-radial-level")), i = q(Number.isFinite(r) ? r : W.length);
		i.length && i[(i.indexOf(document.activeElement) + e + i.length) % i.length].focus();
	}, [q, W.length]), le = (e) => {
		let t = e instanceof Element ? e.closest(".cad-radial-menu__level") : null, n = Number(t?.getAttribute("data-radial-level"));
		return Number.isFinite(n) ? n : W.length;
	};
	if (l(() => () => {
		ne(), K();
	}, [K, ne]), l(() => {
		if (!t) {
			N.current = !1, P.current = null;
			return;
		}
		let e = M?.current;
		return () => {
			j.current && (j.current = !1, window.setTimeout(() => e?.focus?.(), 0));
		};
	}, [M, t]), l(() => {
		if (!t) return;
		let e = P.current;
		if (N.current || (N.current = !0, e = { depth: W.length }), !e) return;
		P.current = null;
		let n = window.setTimeout(() => J(e.depth, e.itemId), 0);
		...
		let e = (e) => {
			let t = e.target;
			!(t instanceof Node) || k.current?.contains(t) || Y(e, "outside");
		};
		return document.addEventListener("pointerdown", e), () => document.removeEventListener("pointerdown", e);
	}, [
		k,
	...
	let ue = n || {
		x: 0,
		y: 0
	}, de = `calc(var(--cad-radial-menu-radius) + ${G - 1} * var(--cad-radial-menu-ring-gap))`, fe = Array.from({ length: Math.max(1, W.length + 1) }, () => "var(--cad-radial-menu-radius) + var(--cad-radial-menu-action-size)").join(" + "), pe = V === "rings" ? `calc(var(--cad-radial-menu-root-outer-radius) + var(--cad-radial-menu-action-size) + ${W.length} * var(--cad-radial-menu-level-gap))` : `calc(${fe})`, me = {
		...m,
		"--cad-radial-menu-root-outer-radius": m?.["--cad-radial-menu-root-outer-radius"] || de,
		"--cad-radial-menu-safe-inset": m?.["--cad-radial-menu-safe-inset"] || pe,
		position: "absolute",
		left: ue.x ?? 0,
		top: ue.y ?? 0,
		zIndex: m?.zIndex ?? 40
	}, he = (t, n, r, s = void 0, c = void 0) => {
		let l = ut(t), u = `calc(var(--cad-radial-menu-radius) + ${Math.max(1, Math.ceil(l.length / U)) - 1} * var(--cad-radial-menu-ring-gap))`, d = n.every((e, t) => W[t] === e) ? W[r] : void 0, f = d === void 0 ? -1 : l.findIndex((e, t) => ft(e, t) === d), p = f < 0 ? void 0 : l[f], m = dt(p), h = {
			"--cad-radial-menu-level-outer-radius": u,
			"--cad-radial-menu-level-entry-angle": c ? `${c.angle}deg` : "0deg",
			"--cad-radial-menu-level-entry-radius": c?.radius || "0px",
			"--cad-radial-menu-level-presentation-offset": r === 0 ? "0px" : `calc(${r} * var(--cad-radial-menu-level-gap))`,
			"--cad-radial-menu-level-depth": r
		}, v = r === 0 ? a : Q(s) || a, y = `${A}-level-${r}`;
		return /* @__PURE__ */ _("div", {
			id: y,
			role: "group",
			"aria-label": r === 0 ? i : `${v} submenu`,
			className: X("cad-radial-menu__level", r > 0 && "cad-radial-menu__level--nested"),
			"data-radial-level": r,
			"data-radial-path": n.join("/"),
			style: h,
			children: [
				/* @__PURE__ */ g("span", {
				...
				l.map((t, i) => {
					let a = Q(t) || "CAD action", s = t.checked, c = !!t.disabled, u = dt(t).length > 0, d = ft(t, i), f = [...n, d], p = u && pt(W.slice(0, f.length), f), m = t.type || (t.toggle ? "checkbox" : "action"), h = m === "checkbox" ? "menuitemcheckbox" : m === "radio" ? "menuitemradio" : "menuitem", v = t.icon, y = e.isValidElement(v) ? v : typeof v == "function" ? /* @__PURE__ */ g(v, { size: 20 }) : (typeof v == "string" || typeof v == "number") && v !== "" ? /* @__PURE__ */ g("span", { children: v }) : null, b = ht(i, l.length, U), x = {
						"--cad-radial-menu-angle": `${b.angle}deg`,
						"--cad-radial-menu-counter-angle": `${-b.angle}deg`,
						"--cad-radial-menu-item-radius": b.radius
					};
					return /* @__PURE__ */ _("button", {
						type: "button",
						role: h,
						...
						"data-checked": s ? "true" : "false",
						"data-radial-ring": b.ring,
						"data-radial-item-id": d,
						"data-radial-collector": u ? "true" : void 0,
						"aria-checked": h === "menuitem" ? void 0 : !!s,
						"aria-haspopup": u ? "menu" : void 0,
						"aria-expanded": u ? p : void 0,
						"aria-controls": u && p ? `${A}-level-${r + 1}` : void 0,
						"aria-label": t["aria-label"] || (u ? `${a}, submenu` : a),
						...
						className: X("cad-radial-menu__item", u && "cad-radial-menu__item--collector", t.className),
						style: x,
						onPointerEnter: (e) => {
							u && oe(t, f, e);
						},
						onKeyDown: (e) => {
							!u || e.key !== "Enter" && e.key !== " " && e.key !== "ArrowRight" || (e.preventDefault(), e.stopPropagation(), ne(), K(), ie(t, f, e, {
								focusChild: !0,
								reason: "keyboard"
							}));
						},
						onClick: (e) => {
							...
							j.current = !1, Ge(t, e, o), Y(e, "action", t);
						},
						children: [
							y && /* @__PURE__ */ g("span", {
							...
							u && /* @__PURE__ */ g("span", {
								className: "cad-radial-menu__collector-mark",
								"aria-hidden": "true",
								children: "⌁"
							}),
							t.shortcut && /* @__PURE__ */ g(Ke, { shortcut: t.shortcut })
						]
					}, t.id || `${a}-${i}`);
				}),
				p && m.length && he(m, [...n, ft(p, f)], r + 1, p, ht(f, l.length, U))
			]
		}, `${n.join("/") || "root"}-${r}`);
	};
	return /* @__PURE__ */ _("div", {
		...O,
		ref: k,
		className: X("cad-radial-menu", `cad-radial-menu--${V}`, te > 1 && "cad-radial-menu--multi-ring", f),
		style: me,
		role: "menu",
		"aria-label": i,
		"data-count": L.length,
		"data-expanded-depth": W.length,
		"data-ring-count": G,
		"data-maximum-ring-count": te,
		"data-presentation": V,
		onPointerEnter: () => K(),
		onPointerLeave: (e) => {
			let t = e.relatedTarget;
			t instanceof Node && e.currentTarget.contains(t) || se(e);
		},
		onKeyDown: (e) => {
			if (y?.(e), !e.defaultPrevented) {
				if ((e.key === "ArrowRight" || e.key === "ArrowDown") && (e.preventDefault(), ce(1, e.target)), (e.key === "ArrowLeft" || e.key === "Backspace") && le(e.target) > 0 ? (e.preventDefault(), e.stopPropagation?.(), ae(e)) : e.key === "ArrowLeft" || e.key === "ArrowUp" ? (e.preventDefault(), ce(-1, e.target)) : e.key === "Backspace" && e.preventDefault(), e.key === "Home" && (e.preventDefault(), J(le(e.target))), e.key === "End") {
					e.preventDefault();
					let t = q(le(e.target));
					t[t.length - 1]?.focus();
				}
				e.key === "Escape" && (e.preventDefault(), e.stopPropagation?.(), ae(e) || Y(e, "escape"));
			}
		},
		children: [he(L, [], 0), h]
	});
}
function vt({ items: e = [], label: t = "More options", open: n, defaultOpen: r = !1, onOpenChange: i, onAction: a, className: o, triggerLabel: s = "More", ...c }) {
...
var yt = (e, t) => Number.isFinite(Number(e)) ? Number(e) : t, bt = (e) => M(e?.tabId || e?.tab || e?.placement?.tab), xt = (e, t) => M(e?.groupId || e?.group || e?.placement?.groupId || e?.placement?.group) || t, St = (e, t) => M(e?.groupLabel || e?.placement?.groupLabel || e?.placement?.group) || t, Ct = (e, t) => yt(e?.order ?? e?.placement?.order, t), wt = (e) => M(e?.tabId || e?.tab || e?.placement?.tab), Tt = (e) => Z(e?.commands).length ? Z(e.commands) : Z(e?.items), Et = (e) => M(e).replace(/[^a-zA-Z0-9_-]+/g, "-") || "workspace", Dt = (e, t) => {
	if (!e || !t) return !1;
	try {
...
function At(e = [], { tabId: t = "", defaultGroupId: n = "commands", defaultGroupLabel: r = "COMMANDS" } = {}) {
	let i = /* @__PURE__ */ new Map();
	return Z(e).forEach((e, a) => {
		if (!e || typeof e != "object") return;
		let o = bt(e);
		if (t && o && o !== t) return;
		let s = xt(e, n), c = St(e, r), l = Ct(e, a), u = i.get(s);
		if (u) {
			u.commands.push({
				command: e,
				order: l,
...
var jt = ({ groups: e, commands: t, activeTabId: n, defaultGroupId: r, defaultGroupLabel: i }) => {
	let a = Z(e).filter((e) => e && typeof e == "object" && (!n || !wt(e) || wt(e) === n)).map((e, t) => ({
		id: M(e.id) || `group-${t + 1}`,
		label: M(e.label) || i,
		order: yt(e.order, t),
		commands: Tt(e).filter((e) => !n || !bt(e) || bt(e) === n)
	})).filter((e) => e.commands.length);
	return a.length ? a.sort((e, t) => e.order - t.order) : At(t, {
		tabId: n,
		defaultGroupId: r,
		defaultGroupLabel: i
	});
...
function Nt({ command: e, group: t, activeTab: n, compact: r, renderIcon: i, renderCommand: a, onCommand: o }) {
	...
	return /* @__PURE__ */ _("button", {
		...
		children: [
			...
			e?.shortcut && /* @__PURE__ */ g(Ke, { shortcut: e.shortcut })
		]
	});
}
function Pt({ tabs: t = [], activeTab: n, defaultActiveTab: r, onActiveTabChange: i, groups: a, commands: o = [], defaultGroupId: s = "commands", defaultGroupLabel: c = "COMMANDS", label: f = "CAD workspace ribbon", tabListLabel: h = "Workspace commands", minimized: v, defaultMinimized: y = !1, onMinimizedChange: b, collapsible: x = !0, compact: S = !1, identity: C, renderIdentity: w, status: T, statusLabel: E = "Workspace status", renderStatus: D, endSlot: O, renderIcon: ee, renderCommand: k, renderMinimizeControl: A, onCommand: j, className: N, style: P, children: F, ...I }) {
	let L = `cad-workspace-ribbon-${Et(u())}`, R = p(/* @__PURE__ */ new Map()), z = p(null), B = p(!1), V = p(""), H = p({
		pointer: !1,
		focus: !1
	}), [U, W] = m(!1), G = d(() => Z(t).filter((e) => e && M(e.id)).map((e) => ({
		...e,
		id: M(e.id),
		label: Q(e) || M(e.id)
	})), [t]), te = G.find((e) => !e.disabled)?.id || G[0]?.id || "", [ne, K] = $(n, r || te, (e, t) => i?.(e, G.find((t) => t.id === e), t)), q = G.find((e) => e.id === ne) || G.find((e) => !e.disabled) || G[0] || null, J = q?.id || "", [Y, re] = $(v, y, (e, t) => b?.(!!e, t));
	l(() => {
		Y || (W(!1), H.current = {
			pointer: !1,
			focus: !1
	...
	let ie = d(() => jt({
		...
		minimized: !!Y,
		flyoutOpen: !!(Y && U)
	}, oe = typeof w == "function" ? w(ae) : C, se = typeof D == "function" ? D(ae) : T, ce = `${L}-panel-${Et(J || "commands")}`, le = (e, t) => {
		e.disabled || e.id === J || V.current === e.id || (V.current = e.id, K(e.id, t), Ot(() => {
			V.current === e.id && (V.current = "");
		}));
	}, ue = (e, t) => {
		if (!(!Y || e.disabled)) {
			if (t?.type === "focus" && B.current) {
				B.current = !1;
				return;
			}
			e.id !== J && le(e, t), W(!0);
		...
		Ot(() => z.current?.querySelector(kt)?.focus());
	}, fe = ({ restoreTabFocus: e = !1 } = {}) => {
		W(!1), !(!e || typeof window > "u") && (B.current = !0, Ot(() => {
			let e = R.current.get(J);
			e ? e.focus() : B.current = !1;
		}));
	}, pe = (e, t, n) => {
		let r = G.filter((e) => !e.disabled);
		if (!r.length) return;
		let i = r[(Math.max(0, r.findIndex((t) => t.id === e)) + t + r.length) % r.length];
		n.preventDefault(), le(i, n), R.current.get(i.id)?.focus();
	}, me = (e, t) => {
		if (Y && t.key === "ArrowDown") {
		...
		if ((t.key === "ArrowRight" || t.key === "ArrowDown") && pe(e.id, 1, t), (t.key === "ArrowLeft" || t.key === "ArrowUp") && pe(e.id, -1, t), t.key === "Home" && pe(G.find((e) => !e.disabled)?.id || e.id, 0, t), t.key === "End") {
			let e = G.filter((e) => !e.disabled).at(-1);
			if (!e) return;
		...
		"aria-label": Y ? "Expand ribbon" : "Minimize ribbon",
		"aria-expanded": !Y,
		title: Y ? "Expand ribbon" : "Minimize ribbon",
		onClick: he,
		children: [/* @__PURE__ */ g("span", {
			"aria-hidden": "true",
			children: Y ? "⌄" : "⌃"
		}), /* @__PURE__ */ g("b", { children: Y ? "EXPAND" : "COMPACT" })]
	}), _e = (e) => /* @__PURE__ */ _("div", {
		id: ce,
		ref: e ? z : void 0,
		role: "tabpanel",
		"aria-labelledby": J ? `${L}-tab-${Et(J)}` : void 0,
		tabIndex: e ? -1 : 0,
		...
		children: [
			/* @__PURE__ */ g("div", {
				...
				role: "group",
				"aria-label": `${q?.label || "CAD"} commands`,
				children: ie.map((e, t) => /* @__PURE__ */ _("section", {
					...
					children: [/* @__PURE__ */ g("div", {
						...
						children: e.commands.map((t, n) => /* @__PURE__ */ g(Nt, {
							command: t,
							group: e,
							activeTab: q,
							compact: S,
		...
		let e = H.current;
		!Y || e.pointer || e.focus || fe();
	}, be = (e) => {
		I.onPointerEnter?.(e), !(e.defaultPrevented || !Y) && (H.current.pointer = !0);
	}, xe = (e) => {
		I.onFocus?.(e), !(e.defaultPrevented || !Y) && (H.current.focus = !0);
	}, Se = (e) => {
		I.onBlur?.(e), !(e.defaultPrevented || !Y || Dt(e.currentTarget, e.relatedTarget)) && (H.current.focus = !1, ye());
	}, Ce = (e) => {
		I.onPointerLeave?.(e), !(e.defaultPrevented || !Y || Dt(e.currentTarget, e.relatedTarget)) && (H.current.pointer = !1, ye());
	};
	return /* @__PURE__ */ _("header", {
		...I,
		className: X("cad-workspace-ribbon", S && "cad-workspace-ribbon--compact", Y && "cad-workspace-ribbon--minimized", N),
		"data-active-tab": J || void 0,
		"data-minimized": Y ? "true" : "false",
		"data-flyout-open": ve ? "true" : "false",
		"aria-label": f,
		style: {
			"--cad-ribbon-accent": q?.color || void 0,
			...P
		},
		onPointerEnter: be,
		onFocus: xe,
		...
		children: [/* @__PURE__ */ _("div", {
			...
			className: X("cad-workspace-ribbon__panel-host", Y && "cad-workspace-ribbon__panel-host--flyout"),
			hidden: !!(Y && !ve),
			children: _e(!!Y)
		})]
	});
...
var Ft = (e, t) => (n) => {
...
}, It = "button:not(:disabled):not([tabindex=\"-1\"]), input:not(:disabled):not([tabindex=\"-1\"]), select:not(:disabled):not([tabindex=\"-1\"]), textarea:not(:disabled):not([tabindex=\"-1\"]), [contenteditable=\"true\"]:not([tabindex=\"-1\"]), [href]:not([tabindex=\"-1\"]), [tabindex]:not([tabindex=\"-1\"])", Lt = (e) => !!(e && !e.hidden && !e.closest?.("[hidden], [aria-hidden=\"true\"], [inert]") && e.getAttribute("aria-hidden") !== "true" && e.getAttribute("aria-disabled") !== "true" && !e.hasAttribute("disabled")), Rt = (e) => [...e?.querySelectorAll(It) || []].filter(Lt), zt = (e) => {
	...
	x: Ht(e?.x, Ht(t?.x, 0)),
	y: Ht(e?.y, Ht(t?.y, 0))
}), Wt = (e, t) => e?.x === t?.x && e?.y === t?.y, Gt = (e, t) => !e || !t ? !1 : e.pointerId === void 0 || t.pointerId === void 0 || e.pointerId === t.pointerId, Kt = (e) => [
	"top",
	"right",
	"bottom",
	"left"
].includes(String(e || "").toLocaleLowerCase()) ? String(e).toLocaleLowerCase() : "right", qt = (e, t, n) => !Number.isFinite(e) || !Number.isFinite(t) ? {
	min: -Infinity,
	max: Infinity
} : e <= t ? {
	min: e,
...
function Jt({ position: e, defaultPosition: t = {
	x: 0,
	y: 0
	...
	let T = u(), E = `cad-movable-overlay-content-${T}`, D = `cad-movable-overlay-instructions-${T}`, O = p(null), ee = p(null), k = p(null), A = p(null), j = p(null), M = p(null), N = p(!1), P = p(!1), [F, I] = m(!1), [L, R] = $(e, Ut(t), (e, t, r) => n?.(e, t, r)), [z, B] = $(r, !!i, (e, t, n) => a?.(e, t, n)), V = Ut(L), H = !!z, U = Kt(d), W = Math.max(1, Math.round(Number(f) || 16));
	j.current = V, M.current = V, N.current = H;
	let G = o(() => {
		...
		let a = n.left - i.x, o = n.top - i.y, s = qt(r.left - a, r.right - a - n.width, i.x), c = qt(r.top - o, r.bottom - o - n.height, i.y);
		return {
			minX: s.min,
			maxX: s.max,
			minY: c.min,
	...
	}, []), te = o((e, t, n = "programmatic", r = {}) => {
		let i = j.current || V, a = Ut(typeof e == "function" ? e(i) : e, i), o = G(), s = {
		...
		return a.changed && (N.current = i, B(i, a, t)), a;
	}, [U, B]), K = o((e) => {
		try {
		...
		let t = k.current;
		if (!t || !Gt(t, e) || e.defaultPrevented) return;
		...
		!t.moved && Math.hypot(i, a) >= 3 && (t.moved = !0, I(!0), s?.(t.startPosition, {
			edge: U,
			source: "pointer"
		}, e)), t.moved && (e.cancelable && e.preventDefault(), te({
		...
		let n = k.current;
		if (!n || e && !Gt(n, e) || (k.current = null, q(), K(n), I(!1), P.current = !!n.moved, !n.moved)) return;
		let r = j.current || n.startPosition;
		c?.(r, {
			changed: !Wt(n.startPosition, r),
			cancelled: !!t,
			edge: U,
			source: "pointer"
		}, e);
		...
		if (!Number.isFinite(n) || !Number.isFinite(r)) return;
		let i = e.currentTarget, a = j.current || V;
		k.current = {
			pointerId: t,
			handle: i,
			startClientX: n,
		...
		let t = e.shiftKey ? 4 : 1, n = W * t, r = G(), i = j.current || V, a;
		e.key === "ArrowLeft" && (a = {
			...i,
			x: i.x - n
		}), e.key === "ArrowRight" && (a = {
			...
			x: r.maxX,
			y: r.maxY
		}), a && (e.preventDefault(), te(a, e, "keyboard", {
			key: e.key,
			multiplier: t
		}));
	}, ae = (e) => {
	...
	l(() => () => {
		let e = k.current;
		k.current = null, q(), K(e);
	}, [K, q]), l(() => {
		!H || typeof document > "u" || document.getElementById(E)?.contains(document.activeElement) && zt(ee.current);
	}, [E, H]), l(() => {
		let e = O.current, t = e?.parentElement;
		if (!e || !t) return;
		let n = () => te(j.current || V, void 0, "boundary");
	...
	return /* @__PURE__ */ _("aside", {
		...
		children: [
			...
			/* @__PURE__ */ _("button", {
				...
				onPointerMove: J,
				onPointerUp: (e) => Y(e, !1),
				onPointerCancel: (e) => Y(e, !0),
				onLostPointerCapture: (e) => Y(e, !0),
				onKeyDown: ie,
				onClick: ae,
...
function Yt({ open: e = !1, onClose: t, title: n, description: r, actions: i, tone: a = "neutral", closeOnBackdrop: o = !0, closeOnEscape: s = !0, className: c, children: d, ...f }) {
	...
	if (b.current = t, x.current = s, l(() => {
		...
		let t = document.activeElement, n = () => {
			...
			if (!Vt(e)) return;
			let t = Rt(e);
			zt(t.find((e) => e.hasAttribute("data-autofocus")) || t[0] || e);
		}, r = (e) => {
			let t = y.current;
			if (e.defaultPrevented || !Vt(t)) return;
			...
			t?.contains(a) ? e.shiftKey && a === r ? (e.preventDefault(), zt(i)) : !e.shiftKey && a === i && (e.preventDefault(), zt(r)) : (e.preventDefault(), zt(e.shiftKey ? i : r));
		}, i = window.setTimeout(n, 0);
		return window.addEventListener("keydown", r), () => {
			window.clearTimeout(i), window.removeEventListener("keydown", r), zt(t);
		};
	}, [e]), !e) return null;
...
function Xt({ open: e, title: t = "Confirm action", description: n, confirmLabel: r = "Confirm", cancelLabel: i = "Cancel", destructive: a = !1, onConfirm: o, onCancel: s, children: c, className: l, ...u }) {
...
function Qt({ toasts: e = [], onDismiss: t, placement: n = "bottom-right", label: r = "Notifications", className: i, ...a }) {
	return /* @__PURE__ */ g("section", {
		...a,
		className: X("cad-toast-stack", `cad-toast-stack--${n}`, i),
		"aria-label": r,
		"aria-live": "polite",
		children: Z(e).map((e, n) => /* @__PURE__ */ g(Zt, {
			toast: e,
			onDismiss: t
		}, e?.id || n))
	});
}
function $t({ trigger: e, content: n, open: r, defaultOpen: i = !1, onOpenChange: o, placement: s = "bottom-start", label: c = "More options", contentRole: d = "region", closeOnOutside: f = !0, closeOnEscape: m = !0, closeOnFocusOutside: h = !1, closeOnPointerLeave: v = !1, restoreFocus: y = !0, focusOnOpen: b, className: x, contentClassName: S, onKeyDown: C, onBlur: w, onPointerLeave: T, ...E }) {
	...
	let L = a(e) ? t(e, {
		...
		w?.(e), !e.defaultPrevented && h && A && !Bt(e.currentTarget, e.relatedTarget) && F(e);
	}, B = (e) => {
		T?.(e), !e.defaultPrevented && v && A && !Bt(e.currentTarget, e.relatedTarget) && F(e);
	};
	return /* @__PURE__ */ _("div", {
		...E,
		ref: O,
...
function en({ content: e, placement: n = "top", className: r, children: i }) {
	let o = u(), [s, c] = m(!1);
	if (!e || !a(i)) return i || null;
	let l = t(i, {
		"aria-describedby": [i.props["aria-describedby"], `cad-tooltip-${o}`].filter(Boolean).join(" "),
		onMouseEnter: Ft(i.props.onMouseEnter, () => c(!0)),
		onMouseLeave: Ft(i.props.onMouseLeave, () => c(!1)),
		onFocus: Ft(i.props.onFocus, () => c(!0)),
		onBlur: Ft(i.props.onBlur, () => c(!1))
	});
	return /* @__PURE__ */ _("span", {
...
function tn({ shortcuts: e = [], title: t = "Keyboard shortcuts", onClose: n, className: r, ...i }) {
	...
	return /* @__PURE__ */ _("section", {
		...
		children: [/* @__PURE__ */ _("header", { children: [/* @__PURE__ */ g("h2", { children: t }), n && /* @__PURE__ */ g("button", {
			...
			children: Object.entries(a).map(([e, t]) => /* @__PURE__ */ _("section", { children: [/* @__PURE__ */ g("h3", { children: e }), /* @__PURE__ */ g("dl", { children: t.map((e) => /* @__PURE__ */ _("div", { children: [
				...
				/* @__PURE__ */ g("dd", { children: /* @__PURE__ */ g(Ke, { shortcut: e.shortcut || e.keys }) }),
				e.detail && /* @__PURE__ */ g("small", { children: e.detail })
			] }, e.id)) })] }, e))
		})]
	});
}
function nn({ open: e = !0, label: t = "Command input", prompt: n, value: r, defaultValue: i = "", onChange: a, onSubmit: o, onCancel: s, placeholder: c, submitLabel: l = "Accept", className: d, ...f }) {
...
var rn = (e, t) => Number.isFinite(Number(e)) ? Number(e) : t, an = (e) => M(e).replace(/[^a-zA-Z0-9_-]+/g, "-") || "workspace", on = (e) => M(e?.tabId || e?.tab || e?.placement?.tab), sn = (e) => M(e?.tabId || e?.tab || e?.placement?.tab), cn = (e) => Z(e?.commands).length ? Z(e.commands) : Z(e?.items), ln = {
	cyan: "#53c9ff",
	green: "#9add4b",
	amber: "#ffb554",
	magenta: "#f08cff",
	violet: "#b9a1ff",
	neutral: "#b4bdc7"
}, un = (e) => e?.color || ln[e?.tone] || "var(--cad-workspace-accent, #53c9ff)", dn = (e) => Z(e).filter((e) => e && M(e.id)).map((e) => ({
	...e,
	id: M(e.id),
	label: Q(e) || M(e.id)
})), fn = ({ groups: e, activeTabId: t, defaultGroupLabel: n }) => Z(e).filter((e) => e && typeof e == "object" && (!t || !sn(e) || sn(e) === t)).map((e, r) => ({
	id: M(e.id) || `group-${r + 1}`,
	label: M(e.label) || n,
	order: rn(e.order, r),
	commands: cn(e).filter((e) => !t || !on(e) || on(e) === t)
})).filter((e) => e.commands.length).sort((e, t) => e.order - t.order);
function pn({ groups: e, commands: t = [], tabId: n = "", defaultGroupId: r = "commands", defaultGroupLabel: i = "COMMANDS" } = {}) {
	let a = fn({
		groups: e,
		activeTabId: n,
		defaultGroupLabel: i
	});
	return a.length ? a : At(t, {
		tabId: n,
		defaultGroupId: r,
		defaultGroupLabel: i
	});
}
function mn({ command: e, group: t, activeTab: n, renderIcon: r, renderCommand: i, onCommand: o, close: s, closeOnCommand: c }) {
...
function hn({ tab: e, groups: t, openGroupId: n, onOpenGroupChange: r, renderIcon: i, renderCommand: a, onCommand: o, close: s, closeOnCommand: c, label: l }) {
		...
		children: [
			...
			d && /* @__PURE__ */ _("div", {
				...
				children: [/* @__PURE__ */ _("div", {
					...
					children: d.commands.map((t, n) => /* @__PURE__ */ g(mn, {
						command: t,
						group: d,
						activeTab: e,
						renderIcon: i,
...
function gn({ tabs: t = [], activeTab: n, defaultActiveTab: r, onActiveTabChange: i, openTabId: o, defaultOpenTabId: s = null, onOpenTabChange: c, openGroupId: l, defaultOpenGroupId: f = null, onOpenGroupChange: m, groups: h, commands: v = [], defaultGroupId: y = "commands", defaultGroupLabel: b = "COMMANDS", label: x = "Compact CAD workspace ribbon", tabListLabel: S = "Compact workspace commands", identity: C, endSlot: w, placement: T = "bottom-start", closeOnOutside: E = !0, closeOnEscape: D = !0, closeOnFocusOutside: O = !0, closeOnPointerLeave: ee = !0, closeOnCommand: k = !0, renderIcon: A, renderCommand: j, onCommand: M, className: N, style: P, ...F }) {
	let I = `cad-compact-workspace-ribbon-${an(u())}`, L = p(/* @__PURE__ */ new Map()), R = d(() => dn(t), [t]), z = R.find((e) => !e.disabled)?.id || R[0]?.id || "", [B, V] = $(n, r || z, (e, t) => i?.(e, R.find((t) => t.id === e) || null, t)), H = R.find((e) => e.id === B) || R.find((e) => !e.disabled) || R[0] || null, [U, W] = $(o, s, (e, t) => c?.(e || null, R.find((t) => t.id === e) || null, t)), G = R.find((e) => e.id === U && !e.disabled) || null, te = G?.id || "", [ne, K] = $(l, f, (e, t, n) => m?.(e || null, t || null, G || null, n)), q = d(() => new Map(R.map((e) => [e.id, pn({
		groups: h,
		commands: v,
		tabId: e.id,
		defaultGroupId: y,
		...
		K(null, null, e), W(null, e);
	}, Y = (e, t) => {
		V(e.id, t), te !== e.id && K(null, null, t), W(e.id, t);
	}, re = (e, t) => {
		e.disabled || (V(e.id, t), J(t));
	}, ie = (e, t, n) => {
		let r = R.filter((e) => !e.disabled);
		if (!r.length) return;
		let i = r[(Math.max(0, r.findIndex((t) => t.id === e)) + t + r.length) % r.length];
		n.preventDefault(), re(i, n), L.current.get(i.id)?.focus();
	}, ae = (e, t) => {
		if ((t.key === "ArrowRight" || t.key === "ArrowDown") && ie(e.id, 1, t), (t.key === "ArrowLeft" || t.key === "ArrowUp") && ie(e.id, -1, t), t.key === "Home") {
			let e = R.find((e) => !e.disabled);
			if (!e) return;
			t.preventDefault(), re(e, t), L.current.get(e.id)?.focus();
		}
		if (t.key === "End") {
	...
	return /* @__PURE__ */ g("header", {
		...
		"data-open-tab": te || void 0,
		"aria-label": x,
		style: {
			"--cad-ribbon-accent": un(H),
			...P
		},
		children: /* @__PURE__ */ _("div", {
			...
			children: [
				...
				R.length > 0 && /* @__PURE__ */ g("div", {
					...
					"aria-label": S,
					children: R.map((t) => {
						let n = t.id === H?.id, r = t.id === te, i = `${I}-tab-${an(t.id)}`, o = q.get(t.id) || [];
						return /* @__PURE__ */ g($t, {
							open: r,
							onOpenChange: (e, n) => {
								e ? Y(t, n) : r && J(n);
							...
							style: { "--cad-compact-ribbon-accent": un(t) },
...
}), xn = (e) => Z(e).find((e) => !e?.disabled)?.id ?? "", Sn = (e, t, n, r) => {
	let i = {
		pan: t.onPan,
		"zoom-in": t.onZoomIn,
...
function Cn({ actions: e = _n, activeId: t, defaultActiveId: n = "", onActiveChange: r, onChange: i, onAction: a, onPan: o, onZoom: s, onZoomIn: c, onZoomOut: l, onZoomWindow: u, onZoomExtents: f, onOrbit: p, onHome: m, label: h = "Viewport navigation", orientation: v = "vertical", className: y, ...b }) {
	let x = d(() => bn(e, "navigation-action"), [e]), [S, C] = $(t, n, (e, t, n) => {
		r?.(e, t, n), i?.(e, t, n);
	}), w = (e, t) => {
		e.disabled || ((e.toggle ?? e.mode ?? !1) && C(S === e.id ? "" : e.id, e, t), e.onClick?.(e, t), a?.(e, t), Sn(e.id, {
			onPan: o,
			onZoom: s,
			onZoomIn: c,
			onZoomOut: l,
...
function wn({ styles: e = vn, value: t, defaultValue: n, onChange: r, onStyleChange: i, label: a = "Visual style", id: o, selectProps: s = {}, disabled: c = !1, className: l, ...f }) {
	let p = u(), m = o || `cad-visual-style-${p}`, h = d(() => bn(e, "visual-style"), [e]), [v, y] = $(t, n ?? h[0]?.id ?? "", (e, t, n) => {
		r?.(e, t, n), i?.(e, t, n);
...
function Tn({ scales: e = yn, value: t, defaultValue: n, onChange: r, onScaleChange: i, onManage: a, manageLabel: o = "Manage", label: s = "Viewport scale", id: c, selectProps: l = {}, disabled: f = !1, className: p, ...m }) {
	let h = u(), v = c || `cad-viewport-scale-${h}`, y = d(() => bn(e, "viewport-scale"), [e]), [b, x] = $(t, n ?? y[0]?.id ?? "", (e, t, n) => {
		r?.(e, t, n), i?.(e, t, n);
...
var Dn = (e, t) => !!e && !!t && e.open === t.open && e.placement === t.placement && e.dockZone === t.dockZone, On = (e) => e instanceof Map ? Object.fromEntries(e.entries()) : N(e) ? e : {}, kn = /* @__PURE__ */ new Set([
	"open",
	"visible",
	"isOpen",
	"placement",
	"mode"
]), An = (e) => N(e) ? Object.fromEntries(Object.entries(e).filter(([e]) => !kn.has(e))) : {}, jn = (...e) => {
	let t = e.find((e) => typeof e == "boolean");
	return t === void 0 ? void 0 : t;
}, Mn = (e, t) => M(e).toLocaleLowerCase().replace(/[^a-z0-9._-]+/g, "-").replace(/^-+|-+$/g, "").slice(0, 80) || t, Nn = Object.freeze({
	DOCK: "dock",
	FLOAT: "float"
}), Pn = Object.freeze({
	LEFT: "left",
...
function Ln(e, t = "") {
	...
	return [
		...
		"command",
		"command-line"
	].includes(n) ? Pn.BOTTOM : t;
}
var Rn = (e) => {
	let t = Z(e?.placements ?? e?.allowedPlacements ?? e?.placementOptions).map((e) => In(e, "")).filter(Boolean), n = !!(e?.preferenceLocked ?? e?.locked), r = !n && e?.dockable !== !1, i = !n && e?.floatable !== !1, a = (t.length ? t : [...r ? [Nn.DOCK] : [], ...i ? [Nn.FLOAT] : []]).filter((e) => e === Nn.DOCK ? r : i);
	return [...new Set(a)];
}, zn = (e) => Array.isArray(e) ? e : e == null ? [] : [e], Bn = (e) => {
	if (e?.dockable === !1) return [];
	let t = zn(e?.dockZones ?? e?.allowedDockZones ?? e?.dockZoneOptions), n = e?.defaultDockZone ?? e?.dockZone ?? e?.zone, r = t.length ? t : n === void 0 ? [] : [n];
...
function Vn(e = []) {
	...
	return Z(e).reduce((e, n, r) => {
		...
		if (t.has(a)) return e;
		t.add(a);
		let o = !!(i.preferenceLocked ?? i.locked), s = Rn(i), c = In(i.defaultPlacement ?? i.placement ?? i.mode, Nn.DOCK), l = s.includes(c) ? c : s[0] || c, u = s.includes(Nn.DOCK) ? Bn(i) : [], d = Ln(i.defaultDockZone ?? i.dockZone ?? i.zone, ""), f = u.includes(d) ? d : u[0] || "", p = jn(i.defaultOpen, i.defaultVisible, i.open, i.visible) ?? !0;
		return e.push({
			...i,
			id: a,
			label: Q(i) || `Panel ${r + 1}`,
...
var Hn = (e, t) => {
	let n = typeof t == "boolean" ? { open: t } : On(t), r = jn(n.open, n.visible, n.isOpen, e.defaultOpen), i = In(n.placement ?? n.mode, e.defaultPlacement), a = e.placements.includes(i) ? i : e.placements[0] || e.defaultPlacement, o = e.dockZones || [], s = Ln(n.dockZone ?? n.zone, e.defaultDockZone), c = o.includes(s) ? s : o[0] || "", l = An(n);
...
function Un(e = [], t = {}) {
	let n = On(t);
	return Vn(e).reduce((e, t) => (e[t.id] = Hn(t, n[t.id]), e), {});
}
function Wn(e = [], t = {}, n) {
	let r = M(n);
	return r ? Un(e, t)[r] : void 0;
}
function Gn(e = [], t = {}) {
	let n = Vn(e), r = Un(n, t), i = {
		[Pn.LEFT]: [],
		[Pn.RIGHT]: [],
		[Pn.BOTTOM]: []
	};
	return n.forEach((e) => {
		...
		!t?.open || t.placement !== Nn.DOCK || !n || i[n].push({
			...e,
			preference: t
		});
	}), i;
}
var Kn = (e) => typeof e == "string" ? { type: e } : N(e) ? e : { type: "" }, qn = (e, t, n) => {
	let { type: r, value: i } = Kn(n), a = { ...t }, o = (t) => e.placements.includes(t), s = (t) => e.dockZones?.includes(t);
	...
	switch (r) {
		...
		case Fn.TOGGLE:
			...
			break;
		case Fn.DOCK:
			if (!o(Nn.DOCK)) return t;
			a.placement = Nn.DOCK;
			break;
		case Fn.FLOAT:
			if (!o(Nn.FLOAT)) return t;
			a.placement = Nn.FLOAT;
			break;
		case Fn.SET_DOCK_ZONE: {
			let e = Ln(i, "");
			if (!o(Nn.DOCK) || !s(e)) return t;
			a.placement = Nn.DOCK, a.dockZone = e;
			break;
		}
		case Fn.RESET: return Hn(e, {});
		case Fn.PATCH: {
			let t = On(i);
			typeof t.open == "boolean" && (t.open || e.closable) && (a.open = t.open);
			let n = In(t.placement ?? t.mode, "");
			n && o(n) && (a.placement = n);
			let r = Ln(t.dockZone ?? t.zone, "");
			r && s(r) && (a.dockZone = r, a.placement = Nn.DOCK);
			break;
		}
		default: return t;
	}
...
function Jn(e = [], t = {}, n, r) {
	let i = M(n), a = Vn(e).find((e) => e.id === i), o = On(t);
	if (!a) return o;
	let s = Hn(a, o[i]), c = qn(a, s, r);
	return Dn(s, c) ? o : {
		...o,
		[i]: c
	};
...
function Xn(e = "cad-workspace", t = "default") {
	let n = N(e) ? e : {
		namespace: e,
		scope: t
	};
...
function Zn({ panels: e = [], value: t, defaultValue: n, onChange: r } = {}) {
	let i = d(() => Vn(e), [e]), [a, s] = $(t, d(() => ({
		...On(n),
		...Un(i, n)
	}), [n, i]), (e, t, n) => {
		r?.(e, t, n);
	...
	return {
		...
		dispatch: o((e, t, n) => {
			...
			let u = Jn(i, a, r, t), d = Un(i, u)[r], f = !Dn(l, d), p = {
				changed: f,
				id: r,
				panel: o,
				action: Kn(t).type,
		...
		reset: o((e) => {
			let t = Yn(i, a), n = Un(i, t), r = i.some((e) => !Dn(c[e.id], n[e.id])), o = {
				changed: r,
				action: Fn.RESET_ALL,
				panels: i,
...
function nr({ panels: t = [], value: n, defaultValue: r, onChange: i, onPanelChange: a, onPanelAction: s, onPanelOpen: c, onPanelClose: l, onPanelDock: f, onPanelDockZone: p, onPanelFloat: m, onPanelReset: v, onResetAll: y, menuOpen: b, defaultMenuOpen: x = !1, onMenuOpenChange: S, title: C = "Workspace panels", description: w = "Show, dock or float the panels used in this workspace.", trigger: T, renderTrigger: E, triggerLabel: D = "Workspace panels", triggerIcon: O = "▦", scope: ee, placement: k = "bottom-end", emptyLabel: A = "No configurable panels are available.", filter: j, defaultFilter: N = "", onFilterChange: P, filterable: F = !0, filterLabel: I = "Find panel", filterPlaceholder: L = "Search panels", clearFilterLabel: R = "Clear panel filter", filteredEmptyLabel: z = "No panels match the current filter.", resetAllLabel: B = "Reset workspace", showResetAll: V = !0, closeLabel: H, renderPanel: U, renderPanelIcon: W, className: G, contentClassName: te, ...ne }) {
	let K = u(), { panels: q, preferences: J, dispatch: Y, reset: re } = Zn({
		...
		le,
		J
	]), de = F && ie.length > 6, fe = o((e, t, n) => {
		let r = Y(e.id, t, n);
		r.changed && (a?.(e.id, r.preference, r, n), s?.(r, n), r.action === Fn.OPEN && c?.(e, r.preference, r, n), r.action === Fn.CLOSE && l?.(e, r.preference, r, n), r.action === Fn.DOCK && f?.(e, r.preference, r, n), r.action === Fn.SET_DOCK_ZONE && p?.(e, r.preference, r, n), r.action === Fn.FLOAT && m?.(e, r.preference, r, n), r.action === Fn.RESET && v?.(e, r.preference, r, n));
	}, [
		Y,
		s,
		a,
		...
		panels: ie,
		preferences: J
	}) : T || me, ge = H || `Close ${C}`, _e = `cad-workspace-panel-manager-${K}`, ve = (e, t) => {
		let n = !!t.open, r = n ? Fn.CLOSE : Fn.OPEN, i = !e.disabled && (!n || e.closable), a = e.placements.length > 1, o = e.placements.includes(Nn.DOCK) && e.dockZones.length > 1, s = {
			open: (t) => fe(e, Fn.OPEN, t),
			close: (t) => fe(e, Fn.CLOSE, t),
			toggle: (t) => fe(e, Fn.TOGGLE, t),
			dock: (t) => fe(e, Fn.DOCK, t),
			dockTo: (t, n) => fe(e, {
				type: Fn.SET_DOCK_ZONE,
				value: t
			}, n),
			float: (t) => fe(e, Fn.FLOAT, t),
			reset: (t) => fe(e, Fn.RESET, t)
		};
		return typeof U == "function" ? U(e, t, s) : /* @__PURE__ */ _("article", {
			...
			children: [
				...
				(a || !e.preferenceLocked) && /* @__PURE__ */ _("div", {
					...
					children: [
						a && e.placements.includes(Nn.DOCK) && /* @__PURE__ */ _("button", {
							type: "button",
							"aria-label": `Dock ${e.label}`,
							"aria-pressed": t.placement === Nn.DOCK,
						...
						a && e.placements.includes(Nn.FLOAT) && /* @__PURE__ */ _("button", {
							type: "button",
							"aria-label": `Float ${e.label}`,
							"aria-pressed": t.placement === Nn.FLOAT,
							disabled: e.disabled || e.preferenceLocked,
							onClick: s.float,
							children: [/* @__PURE__ */ g("span", {
								"aria-hidden": "true",
						...
						a && /* @__PURE__ */ g("output", {
							"aria-label": `${e.label} placement: ${tr(t.placement, t.dockZone).toLocaleLowerCase()}`,
							children: tr(t.placement, t.dockZone)
						}),
				...
				o && /* @__PURE__ */ g("div", {
					...
					children: e.dockZones.map((n) => /* @__PURE__ */ g("button", {
						type: "button",
						"aria-label": `Dock ${e.label} to ${er(n).toLocaleLowerCase()}`,
						"aria-pressed": t.placement === Nn.DOCK && t.dockZone === n,
						disabled: e.disabled || e.preferenceLocked,
	...
	return /* @__PURE__ */ g($t, {
		...ne,
		id: _e,
		className: X("cad-workspace-panel-manager", G),
		contentClassName: X("cad-workspace-panel-manager__surface", te),
		trigger: he,
		open: b,
		defaultOpen: x,
		onOpenChange: S,
			...
			children: [
				...
				ie.length > 0 ? /* @__PURE__ */ _(h, { children: [
					...
					ue.length > 0 ? /* @__PURE__ */ g("div", {
						...
						role: "list",
						children: ue.map((t) => /* @__PURE__ */ g(e.Fragment, { children: ve(t, J[t.id]) }, t.id))
					}) : /* @__PURE__ */ g("p", {
				...
				V && ie.length > 0 && /* @__PURE__ */ _("footer", {
...
var rr = nr, ir = (e) => !!e;
function ar({ active: e, defaultActive: t = !1, onActiveChange: n } = {}) {
	let [r, i] = $(e, ir(t), (e, t, r) => n?.(ir(e), t, r)), a = ir(r), s = o((e, t, n = "programmatic") => {
		let r = ir(typeof e == "function" ? e(a) : e);
		if (r === a) return {
			changed: !1,
			active: a,
			previousActive: a,
...
var or = i(function({ active: e, defaultActive: t = !1, onActiveChange: n, label: r = "Enter focus mode", activeLabel: i = "Exit focus mode", shortcut: a, disabled: o = !1, onClick: s, className: c, title: l, ...u }, d) {
	let f = ar({
		active: e,
		defaultActive: t,
		onActiveChange: n
...
or.displayName = "CadWorkspaceFocusToggle";
//#endregion
//#region src/CadWorkspaceProfiles.ts
var sr = "model", cr = (e) => {
	let t = M(e).toLowerCase();
	return /^[a-z0-9][a-z0-9-]{0,63}$/.test(t) ? t : "";
}, lr = (e, t) => M(e).replace(/\s+/g, " ").slice(0, 48) || t;
function ur(e, { modelId: t = sr, modelName: n = "Model" } = {}) {
	let r = cr(t) || "model", i = Array.isArray(e) ? e : Array.isArray(e?.profiles) ? e.profiles : [], a = /* @__PURE__ */ new Set(), o = i.reduce((e, t, i) => {
		let o = cr(t?.id) || (i === 0 ? r : "");
		return !o || a.has(o) ? e : (a.add(o), e.push({
			...t,
			id: o,
			name: lr(t?.name ?? t?.label, o === r ? n : `Layout ${e.length}`),
			system: o === r || !!t?.system
		}), e);
...
function dr(e, { prefix: t = "Layout", modelId: n = sr } = {}) {
	let r = ur(e, { modelId: n }), i = new Set(r.map((e) => e.name.toLocaleLowerCase())), a = Math.max(1, r.filter((e) => e.id !== n).length + 1), o = `${M(t) || "Layout"} ${a}`;
	for (; i.has(o.toLocaleLowerCase());) a += 1, o = `${M(t) || "Layout"} ${a}`;
	return o;
}
function fr(e, { id: t, name: n, modelId: r = sr, modelName: i = "Model", prefix: a = "Layout", ...o } = {}) {
	let s = ur(e, {
		modelId: r,
		modelName: i
	}), c = new Set(s.map((e) => e.id)), l = cr(t) || "layout", u = l, d = 1;
...
function pr(e, t, n, { modelId: r = sr, modelName: i = "Model" } = {}) {
	let a = cr(t);
	return !a || !M(n) ? ur(e, {
		modelId: r,
		modelName: i
...
function mr(e, t, n, { modelId: r = sr, modelName: i = "Model" } = {}) {
	let a = ur(e, {
		modelId: r,
		modelName: i
	}), o = cr(t), s = o && o !== r ? a.filter((e) => e.id !== o) : a;
	return {
		profiles: s,
...
var hr = (e) => Z(e).find((e) => !e?.disabled)?.id || "", gr = (e, t) => typeof e == "string" ? {
	id: `${e}-${t}`,
	label: e
} : {
	id: e?.id || `${Q(e)}-${t}`,
...
function vr({ items: e = [], activeId: t, defaultActiveId: n, onChange: r, onClose: i, onCreate: a, onContextMenu: o, onRename: s, onOverflow: c, addLabel: l = "New layout", addButtonProps: f = {}, overflowLabel: p = "More drawing spaces", overflowButtonProps: m = {}, ariaLabel: h = "Drawing spaces", className: v, ...y }) {
	let b = u(), x = d(() => Z(e).map((e, t) => ({
		...e,
		id: e?.id || `space-${t}`
	})), [e]), [S, C] = $(t, n || hr(x), (e, t, n) => r?.(e, t, n)), w = x.some((e) => e.id === S) ? S : hr(x), T = (e, t) => {
...
var yr = vr, br = vr;
function xr({ profiles: e = [], activeId: t, onChange: n, onCreate: r, onClose: i, onRename: a, modelId: o = sr, modelName: s = "Model", className: c, ...l }) {
	let u = d(() => ur(e, {
		modelId: o,
		modelName: s
	}), [
...
function Sr({ title: e, icon: t, actions: n, collapsible: r = !1, collapsed: i, defaultCollapsed: a = !1, onCollapsedChange: o, className: s, children: c, ...l }) {
...
function Cr({ items: e = [], activeId: t, defaultActiveId: n, onChange: r, onClose: i, label: a = "Docked panels", compact: o = !1, className: s, children: c, renderPanel: l, ...d }) {
	let f = u(), [p, m] = $(t, n || hr(e), (e, t, n) => r?.(e, t, n)), h = Z(e).find((e) => e?.id === p) || Z(e).find((e) => !e?.disabled), v = (e, t) => {
	...
	return /* @__PURE__ */ _("section", {
		...
		children: [/* @__PURE__ */ g("div", {
			...
			children: Z(e).map((e, t) => {
				let n = e?.id === h?.id, r = e?.icon, a = Q(e), s = o && e?.tabLabel !== void 0 ? e.tabLabel : o && e?.shortLabel !== void 0 ? e.shortLabel : a, c = e?.ariaLabel || e?.accessibleLabel || a, l = e?.title || a, u = _r(e), d = u ? `${c}, ${u.label}` : c;
				return /* @__PURE__ */ _("div", {
...
function wr({ mode: e, label: t, active: n, disabled: r = !1, shortcut: i, tone: a = "inherit", onChange: o, className: s }) {
...
var Tr = (e) => e == null || e === "" ? "" : typeof e == "string" || typeof e == "number" ? String(e) : Array.isArray(e) ? e.map((e, t) => `${"XYZ"[t] || t}: ${e}`).join("  ") : [
	"x",
	"y",
	"z"
].filter((t) => e[t] !== void 0).map((t) => `${t.toUpperCase()}: ${e[t]}`).join("  ");
function Er({ coordinates: e, coordinateLabel: t = "Coordinates", modes: n = [], onModeChange: r, units: i, scale: a, message: o, layout: s = "strip", className: c, children: l, ...u }) {
	let d = Tr(e), f = s === "tiles" || s === "auto" ? s : "strip";
	return /* @__PURE__ */ _("footer", {
		...
		children: [
			...
			/* @__PURE__ */ g("div", {
				...
				children: Z(n).map((e, t) => /* @__PURE__ */ g(wr, {
					mode: e,
					onChange: (t, n, i) => {
...
function Dr({ items: e = [], label: t = "Command history", onSelect: n, className: r }) {
	let i = d(() => Z(e).map(gr), [e]);
...
function Or({ options: e = [], label: t = "Command options", onSelect: n, className: r }) {
	return /* @__PURE__ */ g("div", {
...
function Ar({ value: e, defaultValue: t = "", onChange: n, onSubmit: r, prompt: i = "Command:", history: a = [], suggestions: o = [], options: s = [], onSuggestionSelect: c, onOptionSelect: l, clearOnSubmit: f = !0, submitSuggestionOnEnter: h = !1, disabled: v = !1, placeholder: y = "Type a command or search", showHistory: b = !0, height: x, defaultHeight: S = 152, minHeight: C = 72, maxHeight: w = 360, resizeStep: T = 8, resizable: E = !0, onHeightChange: D, label: O = "CAD command line", className: ee, inputProps: k = {}, style: A, id: j, ...M }) {
	let N = u(), [P, F] = $(e, t, (e, t) => n?.(e, t)), I = Number(C), L = Math.max(48, Number.isFinite(I) ? Math.round(I) : 72), R = Number(w), z = Math.max(L, Number.isFinite(R) ? Math.round(R) : 360), B = kr(S, 152, L, z), [V, H] = $(x, B, (e, t) => D?.(e, t)), U = kr(V, B, L, z), W = Math.max(1, Number.isFinite(Number(T)) ? Math.round(Number(T)) : 8), G = p(null), [te, ne] = m(!1), [K, q] = m(-1), J = d(() => Z(o).map(gr), [o]), Y = `cad-command-suggestions-${N}`, re = j || `cad-command-line-${N}`, ie = (e, t) => {
		let n = kr(typeof e == "function" ? e(U) : e, U, L, z);
		n !== U && H(n, t);
	}, ae = (e) => {
		if (!G.current) return;
		let t = G.current.pointerId;
		G.current = null, e?.currentTarget?.hasPointerCapture?.(t) && e.currentTarget.releasePointerCapture?.(t);
	}, oe = (e) => {
		!E || e.button !== 0 || (e.preventDefault(), G.current = {
		...
		let t = G.current;
		!t || t.pointerId !== e.pointerId || ie(t.startHeight + t.startY - e.clientY, e);
	}, ce = (e, t, n = !1) => {
		e && (F(e.label, t), c?.(e, t), n && (r?.(e.label, t), f && F("", t)), q(-1));
	}, le = (e) => {
		if (e.preventDefault(), K >= 0 && J[K]) {
			ce(J[K], e, h);
			return;
		}
		let t = String(P ?? "").trim();
		t && (r?.(t, e), f && F("", e));
	}, ue = te && J.length > 0, de = s.length > 0 || b && a.length > 0;
	return /* @__PURE__ */ _("section", {
		...M,
		id: re,
		...
		children: [
			E && /* @__PURE__ */ g("div", {
				...
				onPointerCancel: ae,
				onKeyDown: (e) => {
					let t = e.shiftKey ? W * 3 : W;
					e.key === "ArrowUp" && (e.preventDefault(), ie(U + t, e)), e.key === "ArrowDown" && (e.preventDefault(), ie(U - t, e)), e.key === "PageUp" && (e.preventDefault(), ie(U + t * 3, e)), e.key === "PageDown" && (e.preventDefault(), ie(U - t * 3, e)), e.key === "Home" && (e.preventDefault(), ie(L, e)), e.key === "End" && (e.preventDefault(), ie(z, e));
				}
			}),
			/* @__PURE__ */ _("form", {
				...
				children: [
					...
					/* @__PURE__ */ g("input", {
						...
						autoComplete: "off",
						role: "combobox",
						"aria-autocomplete": J.length ? "list" : void 0,
						"aria-expanded": ue,
						"aria-controls": Y,
						"aria-activedescendant": ue && K >= 0 ? `${Y}-${K}` : void 0,
						onFocus: (e) => {
							ne(!0), k.onFocus?.(e);
						},
						onBlur: (e) => {
						...
						onChange: (e) => {
							F(e.target.value, e), q(-1), k.onChange?.(e);
						},
						onKeyDown: (e) => {
							e.key === "ArrowDown" && J.length && (e.preventDefault(), q((e) => (e + 1) % J.length)), e.key === "ArrowUp" && J.length && (e.preventDefault(), q((e) => (e - 1 + J.length) % J.length)), e.key === "Escape" && (q(-1), ne(!1), e.currentTarget.blur()), k.onKeyDown?.(e);
						}
					}),
					/* @__PURE__ */ g("button", {
						type: "submit",
			...
			ue && /* @__PURE__ */ g("div", {
				...
				children: J.map((e, t) => /* @__PURE__ */ _("button", {
					id: `${Y}-${t}`,
					type: "button",
					role: "option",
					"aria-selected": K === t,
					"data-active": K === t ? "true" : "false",
			...
			de && /* @__PURE__ */ _("div", {
				...
				children: [s.length > 0 && /* @__PURE__ */ g(Or, {
					options: s,
					onSelect: l
				}), b && a.length > 0 && /* @__PURE__ */ g(Dr, {
					items: a,
...
function jr({ activeView: e = "top", onViewChange: t, className: n, label: r = "View cube" }) {
	return /* @__PURE__ */ _("div", {
...
function Pr({ activeView: e, onViewChange: t, onZoomIn: n, onZoomOut: r, onZoomExtents: i, showCube: a = !0, showUcs: o = !0, collapsible: s = !1, collapsed: c, defaultCollapsed: d = !1, onCollapsedChange: f, peekOpen: m, defaultPeekOpen: h = !1, onPeekOpenChange: v, peekOnHover: y = !0, peekOnFocus: b = !0, className: x, label: S = "Viewport controls", panelLabel: C, onPointerEnter: w, onPointerLeave: T, onFocusCapture: E, onBlurCapture: D, onKeyDown: O, ...ee }) {
	let k = u(), A = `cad-viewport-controls-content-${k}`, j = `cad-viewport-controls-instructions-${k}`, M = p(null), N = p({
		...
		!R || !z || N.current.dismissed || G(!0, e, t);
	}, ne = (e, t) => {
		let n = N.current;
		!R || !z || n.pointer || n.focus || (n.dismissed = !1, G(!1, e, t));
	}, K = (e) => {
		w?.(e), !(e.defaultPrevented || !R) && (N.current.pointer = !0, N.current.dismissed = !1, y && te(e, "pointer-enter"));
	}, q = (e) => {
		T?.(e), !(e.defaultPrevented || !R || Nr(e.currentTarget, e.relatedTarget)) && (N.current.pointer = !1, ne(e, "pointer-leave"));
	}, J = (e) => {
		E?.(e), !(e.defaultPrevented || !R) && (N.current.focus = !0, N.current.dismissed = !1, b && te(e, "focus-enter"));
	}, Y = (e) => {
		D?.(e), !(e.defaultPrevented || !R || Nr(e.currentTarget, e.relatedTarget)) && (N.current.focus = !1, ne(e, "focus-leave"));
	}, re = (e) => {
		let t = !z;
		t ? (N.current.dismissed = !0, G(!1, e, "collapse")) : G(!1, e, "pin-open"), W(t, e, "toggle");
	}, ie = (e) => {
		O?.(e), !(e.defaultPrevented || e.key !== "Escape" || !z || !B) && (e.preventDefault(), N.current.dismissed = !0, N.current.focus = !1, G(!1, e, "escape"), M.current?.focus());
	};
	l(() => {
		if (!(V || typeof document > "u") && document.getElementById(A)?.contains(document.activeElement)) try {
			M.current?.focus?.({ preventScroll: !0 });
		} catch {
			M.current?.focus?.();
		}
	}, [A, V]);
	let ae = z ? `Open ${H}` : `Collapse ${H}`, oe = z ? V ? `Keep ${H} open` : `Open ${H}` : `Collapse ${H}`;
	return /* @__PURE__ */ _("aside", {
		...ee,
...
function Fr({ count: e = 0, entityLabel: t = "objects", fields: n = [], emptyLabel: r = "Nothing selected", className: i }) {
...
function Ir({ distance: e, angle: t, area: n, volume: r, className: i, label: a = "Measurement" }) {
	let o = [
		{
			id: "distance",
			label: "D",
...
var Lr = Object.freeze({
	...
	RAIL: "rail",
	CLOSED: "closed"
}), Rr = new Set(Object.values(Lr)), zr = /* @__PURE__ */ new Set([
	"left",
	"right",
	"top",
	"bottom"
...
}, Hr = (e, t, n, r) => We(Math.round(Vr(e, t)), n, r), Ur = (e, t) => {
	let n = Math.max(0, Math.round(Vr(e, 72)));
	return {
		minimum: n,
		maximum: Math.max(n, Math.round(Vr(t, 720)))
	};
}, Wr = (e) => Math.max(1, Math.round(Vr(e, 16))), Gr = (e, t = Lr.OPEN) => {
	let n = String(e ?? "").trim().toLocaleLowerCase();
	return Rr.has(n) ? n : t;
}, Kr = (e, t = "always") => {
	let n = String(e ?? "").trim().toLocaleLowerCase();
	return n === "when-open" || n === "always" ? n : t;
}, qr = (e) => zr.has(e) ? e : "left", Jr = (e) => {
	let t = qr(e), n = t === "left" || t === "right", r = t === "left" || t === "top";
	return {
		edge: t,
		axis: n ? "x" : "y",
...
}, Yr = (e, t) => e ? e.pointerId === null || t?.pointerId === null || t?.pointerId === void 0 || t.pointerId === e.pointerId : !1;
function Xr({ mode: e, defaultMode: t = Lr.OPEN, onModeChange: n, size: r, defaultSize: i = 280, minSize: a = 72, maxSize: s = 720, onSizeChange: c } = {}) {
	let { minimum: l, maximum: u } = Ur(a, s), d = Gr(t), f = Hr(i, 280, l, u), [p, m] = $(e, d, (e, t, r) => n?.(Gr(e, d), t, r)), [h, g] = $(r, f, (e, t, n) => c?.(Hr(e, f, l, u), t, n)), _ = Gr(p, d), v = Hr(h, f, l, u), y = o((e, t, n = "programmatic") => {
		let r = Gr(typeof e == "function" ? e(_) : e, _), i = {
			changed: r !== _,
			mode: r,
			previousMode: _,
			source: n
	...
	return {
		...
		setSize: o((e, t, n = "programmatic", r = {}) => {
			let i = Hr(typeof e == "function" ? e(v) : e, v, l, u), a = {
				changed: i !== v,
				size: i,
				previousSize: v,
				minSize: l,
...
function Zr({ mode: e, defaultMode: t = Lr.OPEN, onModeChange: n, label: r = "Workspace dock", controls: i, disabled: a = !1, openDisabled: o = !1, railDisabled: s = !1, hideDisabled: c = !1, openLabel: l, railLabel: u, hideLabel: d, onOpenClick: f, onRailClick: p, onHideClick: m, className: h, "aria-label": v, "aria-controls": y, ...b }) {
	let x = Xr({
		mode: e,
		defaultMode: t,
		onModeChange: n
...
var Qr = i(function({ size: e, defaultSize: t = 280, minSize: n = 72, maxSize: r = 720, resizeStep: i = 16, edge: a = "left", onSizeChange: s, onResizeStart: c, onResizeEnd: u, disabled: f = !1, label: h = "dock", separatorLabel: _, controls: v, className: y, children: b, onPointerDown: x, onPointerMove: S, onPointerUp: C, onPointerCancel: w, onLostPointerCapture: T, onKeyDown: E, "aria-label": D, "aria-controls": O, ...ee }, k) {
	let A = Xr({
		...
		maxSize: r,
		onSizeChange: s
	}), j = d(() => Jr(a), [a]), M = Wr(i), N = p(null), P = p(null), F = p(A.size), I = p(A.setSize), L = p(c), R = p(u), [z, B] = m(!1);
	...
	let V = o((e) => {
		...
		let t = N.current;
		if (!t || !Yr(t, e) || e.defaultPrevented) return;
		let n = t.axis === "x" ? Number(e.clientX) : Number(e.clientY);
		if (!Number.isFinite(n)) return;
		let r = (n - t.startCoordinate) * (t.growsWithPositiveMovement ? 1 : -1), i = Hr(t.startSize + r, t.startSize, t.minSize, t.maxSize);
		...
		let n = N.current;
		if (!n || !Yr(n, e)) return;
		N.current = null, U(), V(n), B(!1);
		let r = Hr(F.current, n.startSize, n.minSize, n.maxSize);
	...
	}, [V, U]), G = o((e) => W(e, !1), [W]), te = o((e) => W(e, !0), [W]);
	l(() => () => {
		let e = N.current;
		if (!e) return;
		N.current = null, U(), V(e);
		let t = Hr(F.current, e.startSize, e.minSize, e.maxSize);
		R.current?.(t, {
	...
	let ne = (e) => {
		...
		let n = Hr(F.current, A.size, A.minSize, A.maxSize), r = Hr(n + e, n, A.minSize, A.maxSize);
		...
		if (e.key === "Home") {
			e.preventDefault(), q("min", e);
			return;
		}
		e.key === "End" && (e.preventDefault(), q("max", e));
	}, Y = O || v, re = D || _ || `Resize ${h}`;
	return /* @__PURE__ */ g("div", {
		...ee,
		ref: k,
...
Qr.displayName = "CadWorkspaceDockResizeHandle";
var $r = (e) => Br.has(e) ? e : "left";
function ei({ peekOpen: e, defaultPeekOpen: t = !1, onPeekOpenChange: n, edge: r = "left" } = {}) {
	let i = $r(r), [a, s] = $(e, !!t, (e, t, r) => n?.(!!e, t, r)), c = !!a, l = o((e, t, n = "programmatic") => {
...
function ni({ edge: e = "left", label: t = "Workspace dock", previewLabel: n, expandLabel: r, children: i, renderPreview: a, previewMount: o, peekOpen: s, defaultPeekOpen: c = !1, onPeekOpenChange: l, onExpand: f, disabled: m = !1, id: h, controls: v, className: y, railClassName: b, previewClassName: x, onPointerEnter: S, onPointerLeave: C, onFocusCapture: w, onBlurCapture: T, onKeyDown: E, "aria-label": D, ...O }) {
	let ee = u(), k = p(null), A = p({
		pointer: !1,
		focus: !1,
		dismissed: !1
	}), j = $r(e), M = h || `cad-workspace-dock-rail-${ee}`, N = `${M}-label`, P = `${M}-preview`, F = ei({
		edge: j,
		peekOpen: s,
		defaultPeekOpen: c,
		onPeekOpenChange: l
	}), I = !m && F.peekOpen, L = String(t || "Workspace dock"), R = n || `${L} preview`, z = r || `Expand ${L}`, B = typeof a == "function" ? a : typeof i == "function" ? i : null, V = Kr(o, B ? "when-open" : "always"), H = d(() => ({
		active: I,
		peekOpen: I,
		edge: j,
		label: L,
		...
		j,
		L
	]), U = I || V === "always", W = U ? B ? B(H) : i : null, G = (e, t) => {
		m || (A.current.dismissed = !1, F.openPeek(e, t));
	}, te = (e, t) => {
		let n = A.current;
		m || n.pointer || n.focus || (n.dismissed = !1, F.closePeek(e, t));
	}, ne = (e) => {
		S?.(e), !(e.defaultPrevented || m) && (A.current.pointer = !0, G(e, "pointer-enter"));
	}, K = (e) => {
		C?.(e), !(e.defaultPrevented || m || ti(e.currentTarget, e.relatedTarget)) && (A.current.pointer = !1, te(e, "pointer-leave"));
	}, q = (e) => {
		w?.(e), !(e.defaultPrevented || m) && (A.current.focus = !0, A.current.dismissed || G(e, "focus-enter"));
	}, J = (e) => {
		T?.(e), !(e.defaultPrevented || m || ti(e.currentTarget, e.relatedTarget)) && (A.current.focus = !1, te(e, "focus-leave"));
	}, Y = (e) => {
...
function ri({ edge: e = "left", panels: t = [], activeId: n, defaultActiveId: r, onActiveChange: i, onPanelClose: a, label: s = "Docked panels", tabsLabel: c, compactTabs: l = !1, renderPanel: u, children: d, id: f, className: p, tabsClassName: m, panelClassName: h, emptyLabel: _ = "No panels are available in this dock.", ...v }) {
	let y = $r(e), b = Z(t), x = o((e) => {
	...
	return /* @__PURE__ */ g("section", {
		...
		"aria-label": s,
		role: "complementary",
		children: b.length > 0 ? /* @__PURE__ */ g(Cr, {
			items: b,
			activeId: n,
			defaultActiveId: r,
...
]), li = (e) => Z(e).map((e, t) => typeof e == "string" ? {
	id: e,
	label: e
} : {
	...e,
...
function ui({ mode: e = "point", fields: t, value: n, defaultValue: r = {}, onChange: i, onSubmit: a, prompt: o = "Specify point", unit: s = "mm", visible: c = !0, submitLabel: l = "Accept", className: f, children: p, ...m }) {
	let h = u(), v = Z(t).length ? Z(t) : ii[e] || ii.point, y = d(() => v.reduce((e, t) => t?.id && t.value !== void 0 ? {
		...e,
		[t.id]: t.value
	} : e, {}), [v]), [b, x] = $(n, d(() => ({
		...y,
		...
		children: [
			...
			/* @__PURE__ */ _("div", {
				...
				children: [v.map((e, t) => {
					...
					return e.type === "angle" ? /* @__PURE__ */ g(tt, {
						...r,
						unit: e.unit || "°"
					}, n) : e.type === "unit" ? /* @__PURE__ */ g(et, { ...r }, n) : /* @__PURE__ */ g($e, { ...r }, n);
				}), p]
			}),
			a && /* @__PURE__ */ _("button", {
				type: "submit",
...
function di({ modes: e = ai, activeIds: t, defaultActiveIds: n = [], multiple: r = !0, onChange: i, onClose: a, label: o = "Object snaps", className: s, ...c }) {
	let l = d(() => li(e), [e]), [u, f] = $(t, n, (e, t, n) => i?.(e, t, n)), p = new Set(Z(u)), m = (e, t) => {
...
function fi({ tools: e = [], selectionCount: t, label: n = "Selection tools", onAction: r, onDismiss: i, className: a, ...o }) {
...
function pi({ constraints: e = oi, activeIds: t, defaultActiveIds: n = [], onChange: r, onAction: i, label: a = "Geometric constraints", layout: o = "strip", className: s, ...c }) {
	let l = d(() => li(e), [e]), [u, f] = $(t, n, (e, t, n) => r?.(e, t, n)), p = new Set(Z(u)), m = o === "auto" || o === "tiles" ? o : "strip", h = (e, t) => {
		if (e.disabled) return;
...
function mi({ scales: e = si, value: t, defaultValue: n, onChange: r, label: i = "Annotation scale", onManage: a, id: o, selectProps: s = {}, disabled: c = !1, layout: l = "stacked", className: f, ...p }) {
	let m = u(), h = o || `cad-annotation-scale-${m}`, v = d(() => li(e), [e]), [y, b] = $(t, n ?? v[0]?.id ?? "", (e, t, n) => r?.(e, t, n)), x = l === "inline" ? "inline" : "stacked";
	return /* @__PURE__ */ _("div", {
		...p,
...
function hi({ presets: e = ci, value: t, defaultValue: n, onChange: r, label: i = "View preset", id: a, selectProps: o = {}, disabled: s = !1, className: c, ...l }) {
	let f = u(), p = a || `cad-view-preset-${f}`, m = d(() => li(e), [e]), [h, v] = $(t, n ?? m[0]?.id ?? "", (e, t, n) => r?.(e, t, n));
	return /* @__PURE__ */ _("div", {
		...l,
...
function gi({ angle: e, distance: t, increment: n, active: r, defaultActive: i = !1, onActiveChange: a, className: o, label: s = "Polar tracking", ...c }) {
...
function _i({ type: e = "endpoint", label: t, active: n = !0, className: r, style: i, ...a }) {
	let o = ai.find((t) => t.id === e)?.glyph || "•";
	return /* @__PURE__ */ _("span", {
		...a,
...
function vi({ label: e = "Selection grip", variant: t = "square", active: n = !1, disabled: r = !1, onPointerDown: i, onClick: a, className: o, ...s }) {
	return /* @__PURE__ */ g("button", {
		...s,
		type: "button",
...
]), Si = (e) => Z(e).map((e, t) => typeof e == "string" || typeof e == "number" ? {
	id: String(e),
	label: String(e),
	value: e
} : {
...
function Ti({ orientation: e = "horizontal", size: t, defaultSize: n = 30, minSize: r = 12, maxSize: i = 88, keyboardStep: a = 5, primary: o, secondary: s, onSizeChange: c, onResizeStart: u, onResizeEnd: d, separatorLabel: f = "Resize panels", className: m, ...h }) {
		...
		children: [
			...
			/* @__PURE__ */ g("div", {
				...
				onKeyDown: (t) => {
					...
					t.key === "End" && (t.preventDefault(), V(ee, t));
				},
				children: /* @__PURE__ */ g("span", { "aria-hidden": "true" })
			}),
			/* @__PURE__ */ g("div", {
...
function Ei({ item: e, open: t, onToggle: n, onAction: r, onClose: i }) {
	let a = Si(e?.items), o = `cad-menu-bar-popup-${u()}`, s = a.length > 0;
	return /* @__PURE__ */ _("div", {
		...
		children: [/* @__PURE__ */ _("button", {
			...
			children: [Q(e), e?.shortcut && /* @__PURE__ */ g(Ke, { shortcut: e.shortcut })]
		}), t && /* @__PURE__ */ g("div", {
			id: o,
...
function Di({ item: e, onAction: t, onClose: n, className: r }) {
	let i = Si(e?.items), a = i.length > 0, [o, s] = $(void 0, !1);
	return /* @__PURE__ */ _("div", {
...
function Oi({ items: e = [], openId: t, defaultOpenId: n = "", onOpenChange: r, onAction: i, label: a = "CAD application menu", endSlot: s, endSlotLabel: c = "Application controls", className: u, ...f }) {
	let m = d(() => Si(e), [e]), [h, v] = $(t, n, (e, t, n) => r?.(e, t, n)), y = p(null), b = p(""), x = m.find((e) => e.id === h && !e.disabled && Si(e.items).length > 0), S = x?.id || "", C = o((e) => {
		...
		if (!(e?.disabled || Si(e?.items).length === 0)) {
			if (e.id === S) {
				T(e, t);
				return;
			}
	...
	let D = (e, t) => {
		...
		onKeyDown: (e) => {
			...
			if (e.key === "ArrowDown" && t?.dataset.menuId) {
				...
				n && !n.disabled && Si(n.items).length > 0 && (e.preventDefault(), n.id === S ? window.requestAnimationFrame(() => w(n.id)) : (b.current = n.id, v(n.id, n, e)));
			}
		},
		children: m.map((e) => /* @__PURE__ */ g(Ei, {
			item: e,
			open: S === e.id,
			onToggle: E,
			onAction: i,
...
function ki({ value: e, defaultValue: t = { mode: "by-layer" }, onChange: n, colors: r = yi, allowByLayer: i = !0, allowByBlock: a = !0, label: o = "Color", className: s, ...c }) {
	let [l, u] = $(e, t, (e, t) => n?.(e, t)), d = Ci(l), f = (e, t) => u(e, t);
	return /* @__PURE__ */ _("section", {
		...c,
		className: X("cad-color-picker", s),
		"aria-label": o,
		children: [
			/* @__PURE__ */ _("header", { children: [/* @__PURE__ */ g("strong", { children: o }), /* @__PURE__ */ g(rt, {
...
function ji({ linetypes: e = bi, value: t, defaultValue: n, onChange: r, label: i = "Linetype", className: a, ...o }) {
	let s = d(() => Si(e), [e]), [c, l] = $(t, n ?? s[0]?.id ?? "", (e, t, n) => r?.(e, t, n)), u = s.find((e) => e.id === c) || s[0];
	return /* @__PURE__ */ g($t, {
		label: i,
...
function Mi({ lineweights: e = xi, value: t, defaultValue: n, onChange: r, label: i = "Lineweight", className: a, ...o }) {
	let s = d(() => Si(e), [e]), [c, l] = $(t, n ?? s[0]?.id ?? "", (e, t, n) => r?.(e, t, n)), u = s.find((e) => e.id === c) || s[0];
	return /* @__PURE__ */ g($t, {
		label: i,
...
function Ni({ block: e, selected: t = !1, onSelect: n, onInsert: r, onEdit: i, onDelete: a, renderThumbnail: o, className: s }) {
	let c = e || {}, l = Q(c);
	return /* @__PURE__ */ _("article", {
...
function Pi({ blocks: e = [], value: t, defaultValue: n = "", onChange: r, onInsert: i, onCreate: a, onEdit: o, onDelete: s, filter: c, defaultFilter: l = "", onFilterChange: f, view: p = "grid", renderThumbnail: m, title: h = "Blocks", className: v, emptyLabel: y = "No blocks match the current filter" }) {
	...
	return /* @__PURE__ */ _("section", {
		...
		children: [
			...
			/* @__PURE__ */ _("div", {
				...
				role: "list",
				children: [T.map((e, t) => /* @__PURE__ */ g(Ni, {
					block: e,
					selected: e?.id === x,
...
function Fi({ value: e, defaultValue: t = {
	scale: 1,
	rotation: 0,
	uniform: !0,
	specifyOnScreen: !1,
...
function Ii({ value: e, defaultValue: t = "", onChange: n, placeholder: r = "Filter", label: i = "Filter list", className: a, ...o }) {
...
function Li({ property: e, value: t, onValueChange: n, inputId: r, className: i }) {
	...
	}) : o === "scale" ? /* @__PURE__ */ g(mi, {
		value: s,
		onChange: (e, t, n) => c(e, n),
		label: a.label || a.id,
		className: X("cad-property-field", "cad-property-field--style", i),
		scales: a.options
	}) : o === "number" ? /* @__PURE__ */ g($e, {
		id: r,
		className: X("cad-property-field", i),
		value: s,
		min: a.min,
...
function Ri({ property: e, value: t, onValueChange: n, className: r }) {
	let i = u(), a = e || {};
...
function zi({ id: e, title: t, properties: n = [], collapsible: r = !0, open: i, defaultOpen: a = !0, onOpenChange: o, onValueChange: s, className: c, children: l }) {
	...
	return /* @__PURE__ */ _("section", {
		...
		children: [h, /* @__PURE__ */ g("div", {
			...
			hidden: !p,
			children: l || Z(n).map((e, t) => /* @__PURE__ */ g(Ri, {
				property: e,
				onValueChange: s
			}, e?.id || t))
		})]
...
function Bi({ sections: e, properties: t, onValueChange: n, label: r = "Properties", className: i, ...a }) {
	...
	return /* @__PURE__ */ g("section", {
		...
		"aria-label": r,
		children: o.map((e, t) => /* @__PURE__ */ g(zi, {
			...e,
			onValueChange: n
		}, e?.id || t))
	});
}
function Vi({ layers: e = [], value: t, defaultValue: n, onChange: r, label: i = "Current layer", className: a, disabled: o = !1 }) {
...
function Hi({ layer: e, active: t = !1, onActivate: n, onLayerChange: r, onColorClick: i, className: a }) {
...
function Ui({ layers: e = [], activeLayerId: t, onActiveLayerChange: n, onLayerChange: r, onAddLayer: i, onDeleteLayer: a, onColorClick: o, title: s = "Layers", filter: c, defaultFilter: l = "", onFilterChange: u, filterable: f = !0, className: p, emptyLabel: m = "No layers match this filter" }) {
	...
	return /* @__PURE__ */ _("section", {
		...
		children: [
			...
			/* @__PURE__ */ _("div", {
				...
				role: "list",
				children: [y.map((e, i) => /* @__PURE__ */ g(Hi, {
					layer: e,
...
function Wi({ node: e, level: t, selectedId: n, expandedIds: r, onSelect: i, onExpandedChange: a }) {
	...
	return /* @__PURE__ */ _("li", {
		...
		}), c && l && /* @__PURE__ */ g("ul", { children: s.map((e, o) => /* @__PURE__ */ g(Wi, {
			node: e,
			level: t + 1,
			selectedId: n,
			expandedIds: r,
...
function Gi({ nodes: e = [], selectedId: t, defaultSelectedId: n = "", onSelect: r, expandedIds: i, defaultExpandedIds: a, onExpandedChange: o, label: s = "CAD object tree", className: c, ...l }) {
	...
	return /* @__PURE__ */ g("ul", {
		...
		"aria-label": s,
		children: Z(e).map((e, t) => /* @__PURE__ */ g(Wi, {
			node: e,
			level: 1,
			selectedId: d,
			expandedIds: h,
...
function Ki({ label: e, value: t = 0, status: n, onCancel: r, className: i }) {
...
function qi({ references: e = [], onReload: t, onUnload: n, className: r, title: i = "External references" }) {
	return /* @__PURE__ */ _("section", {
...
var Ji = (e, t) => typeof t?.render == "function" ? t.render(e, t) : typeof t?.accessor == "function" ? t.accessor(e, t) : e?.[t?.accessor || t?.id], Yi = (e, t) => {
	let n = typeof t?.sortValue == "function" ? t.sortValue(e, t) : Ji(e, t);
	return typeof n == "string" ? n.toLocaleLowerCase() : n;
};
function Xi({ columns: e = [], rows: t = [], rowId: n = (e) => e?.id, selectedIds: r, defaultSelectedIds: i = [], onSelectionChange: a, selectionMode: o = "multiple", onRowActivate: s, sort: c, defaultSort: l, onSortChange: u, caption: f = "CAD data", emptyLabel: p = "No rows to display", layout: m = "table", className: h, ...v }) {
	let y = d(() => Z(e).filter((e) => e?.id), [e]), [b, x] = $(r, i, (e, t, n) => a?.(e, t, n)), [S, C] = $(c, l, (e, t, n) => u?.(e, t, n)), w = new Set(Z(b)), T = m === "auto" || m === "cards" ? m : "table", E = d(() => {
		...
		return e.sort((e, t) => String(Yi(e, n) ?? "").localeCompare(String(Yi(t, n) ?? ""), void 0, { numeric: !0 }) * r);
	}, [
		S,
		y,
		t
...
function Zi({ filters: e = [], activeIds: t, defaultActiveIds: n = [], onChange: r, label: i = "Selection filter", className: a, ...o }) {
...
function Qi({ candidates: e = [], activeId: t, defaultActiveId: n, onChange: r, onAccept: i, onCancel: a, label: o = "Selection cycle", layout: s = "strip", className: c, ...l }) {
	let u = d(() => Z(e).map((e, t) => ({
		...e,
		id: e?.id || `${Q(e)}-${t}`
...
function $i({ title: e = "Quick properties", properties: t, sections: n, onValueChange: r, onPinChange: i, pinned: a = !1, onClose: o, className: s, ...c }) {
	return /* @__PURE__ */ _("aside", {
		...c,
...
var ea = "cad-cui-workspace-preset", ta = 1, na = Object.freeze({
	...
	"constructor",
	"prototype"
]), ia = Symbol("omit"), aa = (e) => N(e) && (Object.getPrototypeOf(e) === Object.prototype || Object.getPrototypeOf(e) === null), oa = (e) => !ra.has(e), sa = (e) => e instanceof Map ? Object.fromEntries(e.entries()) : N(e) ? e : {}, ca = (e, t) => M(e).replace(/\s+/g, " ").slice(0, 80) || t, la = (e) => M(e).slice(0, 400), ua = (e) => M(e).toLocaleLowerCase().replace(/[^a-z0-9._-]+/g, "-").replace(/^-+|-+$/g, "").slice(0, 80), da = (e, t) => Number.isSafeInteger(e) && e > 0 ? e : t, fa = (e) => {
	let t = sa(e), n = la(t.schema) || "cad-cui-workspace-preset", r = da(t.version, 1), i = ca(t.defaultName, "Workspace"), a = typeof t.normalizePanelPreferences == "function" ? t.normalizePanelPreferences : typeof t.panelPreferenceNormalizer == "function" ? t.panelPreferenceNormalizer : void 0;
	return {
		schema: n,
		version: r,
		defaultName: i,
...
function ma(e, t = /* @__PURE__ */ new Set()) {
	...
	if (typeof e == "number") return Number.isFinite(e) ? e : ia;
	if (typeof e != "object" || t.has(e)) return ia;
	if (Array.isArray(e)) {
		let n = new Set(t);
...
var ha = (e) => {
	let t = ma(sa(e));
	return aa(t) ? t : {};
}, ga = (e) => e.panels ?? e.panelPreferences ?? e.preferences ?? {}, _a = (e) => e.settings ?? e.ui ?? e.state ?? {}, va = (e, t) => {
	if (!t.panelNormalizer) return ha(e);
	try {
		return ha(t.panelNormalizer(t.panels, e));
	} catch {
		return ha(e);
	}
}, ya = (e, t) => {
	let n = sa(e), r = ua(n.id ?? n.presetId), i = la(n.description), a = pa(n.savedAt ?? n.updatedAt), o = {
		schema: t.schema,
		version: t.version,
		name: ca(n.name ?? n.label, t.defaultName),
		panels: va(ga(n), t),
		settings: ha(_a(n)),
		metadata: ha(n.metadata)
	};
	...
	message: t,
	...n ? { path: n } : {}
}), xa = (e, t = "$", n = /* @__PURE__ */ new Set(), r = []) => {
	if (typeof e != "object" || !e || n.has(e)) return r;
	let i = new Set(n);
	return i.add(e), Array.isArray(e) ? (e.forEach((e, n) => xa(e, `${t}[${n}]`, i, r)), r) : (Object.keys(e).forEach((n) => {
		let a = `${t}.${n}`;
		if (!oa(n)) {
			r.push(ba(na.UNSAFE_KEY, `Preset key "${n}" is not allowed.`, a));
			return;
		}
		xa(e[n], a, i, r);
	}), r);
	...
	"settings",
	"metadata"
].reduce((t, n) => (e[n] !== void 0 && !N(e[n]) && t.push(ba(na.INVALID_FIELD, `Preset field "${n}" must be an object.`, `$.${n}`)), t), []);
function Ca(e = {}, t = {}) {
	return ya(e, fa(t));
}
var wa = Ca;
...
function Ea(e, t = {}) {
	...
	if (!N(e)) return {
		ok: !1,
		preset: void 0,
		errors: [ba(na.INVALID_PRESET, "A workspace preset must be a JSON object.", "$")]
	};
	let r = [];
	if (e.schema !== n.schema && r.push(ba(na.UNSUPPORTED_SCHEMA, `Expected preset schema "${n.schema}".`, "$.schema")), (!Number.isSafeInteger(e.version) || e.version !== n.version) && r.push(ba(na.UNSUPPORTED_VERSION, `Expected preset version ${n.version}.`, "$.version")), e.name !== void 0 && typeof e.name != "string" && r.push(ba(na.INVALID_FIELD, "Preset field \"name\" must be a string.", "$.name")), e.id !== void 0 && typeof e.id != "string" && r.push(ba(na.INVALID_FIELD, "Preset field \"id\" must be a string.", "$.id")), e.description !== void 0 && typeof e.description != "string" && r.push(ba(na.INVALID_FIELD, "Preset field \"description\" must be a string.", "$.description")), e.savedAt !== void 0 && (!pa(e.savedAt) || typeof e.savedAt != "string") && r.push(ba(na.INVALID_FIELD, "Preset field \"savedAt\" must be a valid ISO date string.", "$.savedAt")), r.push(...Sa(e), ...xa(e)), r.length) return {
		ok: !1,
		preset: void 0,
		errors: r
	};
	try {
		...
		return {
			ok: !1,
			preset: void 0,
			errors: [ba(na.NORMALIZATION_FAILED, "The workspace preset could not be normalized.")]
		};
	}
}
function Da(e = {}, t = {}) {
	let n = Ea(ya(e, fa(t)), t);
	if (!n.ok) return {
		...n,
		json: void 0
	};
	let r = sa(t), i = r.pretty === !1 ? 0 : Math.max(0, Math.min(10, Number.isFinite(r.space) ? Math.floor(r.space) : 2));
	try {
		return {
			ok: !0,
			preset: n.preset,
		...
		return {
			...
			preset: void 0,
			json: void 0,
			errors: [ba(na.NORMALIZATION_FAILED, "The workspace preset could not be serialized.")]
		};
	}
}
function Oa(e, t = {}) {
	if (typeof e == "string") try {
		return Ea(JSON.parse(e.replace(/^\uFEFF/, "")), t);
	} catch {
		return {
			ok: !1,
			preset: void 0,
			errors: [ba(na.INVALID_JSON, "The workspace preset is not valid JSON.")]
		};
	}
	return N(e) ? Ea(e, t) : {
		ok: !1,
		preset: void 0,
		errors: [ba(na.INVALID_INPUT, "Provide a preset JSON string or parsed object.")]
	};
}
//#endregion
//#region src/CadWorkspacePresetUi.tsx
var ka = (e) => String(e ?? ""), Aa = Object.freeze({
	SELECT: "select",
	...
	EXPORT: "export",
	IMPORT: "import"
}), ja = (e) => Array.isArray(e) ? e : Array.isArray(e?.presets) ? e.presets : [];
function Ma(e = []) {
	let t = /* @__PURE__ */ new Set();
	return ja(e).reduce((e, n, r) => {
		let i = typeof n == "string" || typeof n == "number" ? {
			id: String(n),
			name: String(n)
		} : n;
...
var Fa = (e) => e === "error" || e === "warning" ? "alert" : "status";
function Ia({ presets: e = [], selectedPresetId: t = "", draftName: n = "", onSelectedPresetIdChange: r, onDraftNameChange: i, onSaveAs: a, onLoad: s, onOverwrite: c, onDelete: l, onExport: f, onImport: p, onAction: m, title: v = "Workspace presets", description: y = "Save, restore and exchange workspace arrangements.", presetListLabel: b = "Saved presets", draftNameLabel: x = "Preset name", draftNamePlaceholder: S = "e.g. Focused drafting", saveAsLabel: C = "Save as", loadLabel: w = "Load", overwriteLabel: T = "Overwrite", deleteLabel: E = "Delete", exportLabel: D = "Export", importLabel: O = "Import", selectedLabel: ee = "Selected preset", noSelectionLabel: k = "Choose a saved preset", emptyLabel: A = "No saved presets yet.", emptyStateGuideLabel: j = "First preset checklist", emptyStateGuideSteps: N, duplicateNameLabel: P = "A preset with this name already exists.", readOnlyLabel: F = "Protected preset", importDescription: I = "The host chooses a file and validates its contents.", status: L, statusTone: R = "neutral", busy: z = !1, disabled: B = !1, allowDuplicateNames: V = !1, maxNameLength: H = 64, className: U, children: W, ...G }) {
	let te = u(), ne = `cad-workspace-preset-manager-${te}-title`, K = `cad-workspace-preset-manager-${te}-description`, q = `cad-workspace-preset-manager-${te}-name`, J = `cad-workspace-preset-manager-${te}-list`, Y = `cad-workspace-preset-manager-${te}-status`, re = d(() => Ma(e), [e]), ie = re.length > 0, ae = M(t), oe = d(() => re.find((e) => e.id === ae), [re, ae]), se = M(n), ce = d(() => [
	...
	}, [ce, N]), ue = M(j) || "First preset checklist", de = !V && Pa(re, se), fe = !!(B || z), pe = !fe && ie && typeof r == "function", me = !fe && typeof i == "function", he = !fe && !!se && !de && typeof a == "function", ge = !fe && !!oe && !oe.disabled && typeof s == "function", _e = !fe && !!oe?.canOverwrite && typeof c == "function", ve = !fe && !!oe?.canDelete && typeof l == "function", ye = !fe && ie && typeof f == "function", be = !fe && typeof p == "function", xe = o((e, t, n = {}) => {
		let r = {
			type: e,
		...
		let t = e.target.value, n = xe(Aa.DRAFT_NAME_CHANGE, e, { name: M(t) });
		i?.(t, n, e);
	}, [i, xe]), Ce = o((e) => {
		let t = M(e.target.value), n = re.find((e) => e.id === t), i = xe(Aa.SELECT, e, {
			selectedPresetId: t,
			preset: n
		});
		r?.(t, n, i, e);
		...
		if (!be) return;
		let t = xe(Aa.IMPORT, e);
		p?.(t, e);
	}, [
		be,
		p,
		xe
	]), Ae = [y ? K : "", L ? Y : ""].filter(Boolean).join(" ") || void 0;
	return /* @__PURE__ */ _("section", {
		...G,
				...
				children: [
					...
					/* @__PURE__ */ _("div", {
						...
						children: [/* @__PURE__ */ g("input", {
							...
							"aria-describedby": de ? `${q}-duplicate` : void 0,
							onChange: Se
						}), /* @__PURE__ */ g("button", {
							type: "submit",
							disabled: !he,
...
var La = Ia, Ra = (e, t) => M(e?.id ?? e?.key) || `control-${t + 1}`, za = (e, t) => M(e?.ariaLabel ?? e?.accessibleLabel ?? Q(e)) || `Workspace control ${t + 1}`, Ba = (e) => e?.active !== void 0 || e?.pressed !== void 0, Va = (e) => !!(e?.active ?? e?.pressed), Ha = (t, n) => e.isValidElement(t?.icon) ? t.icon : typeof t?.icon == "function" ? e.createElement(t.icon, {
	size: 14,
	"aria-hidden": !0
...
}), Ua = i(function({ items: e = [], label: t = "Workspace controls", onItemClick: n, className: r, style: i, role: a = "group", ...o }, s) {
	let c = d(() => Z(e).filter((e) => e && typeof e == "object").map((e, t) => ({
		item: e,
		index: t,
		id: Ra(e, t),
		accessibleLabel: za(e, t),
		activeState: Va(e),
		mode: M(e.mode),
		shortcut: M(e.shortcut),
		toggle: Ba(e)
...
Ua.displayName = "CadWorkspaceChromeControls";
//#endregion
export { G as CAD_CUI_RUNTIME_VERSION, Lr as CAD_WORKSPACE_DOCK_MODES, sr as CAD_WORKSPACE_MODEL_ID, Fn as CAD_WORKSPACE_PANEL_ACTIONS, Pn as CAD_WORKSPACE_PANEL_DOCK_ZONES, Nn as CAD_WORKSPACE_PANEL_PLACEMENTS, Aa as CAD_WORKSPACE_PRESET_ACTIONS, na as CAD_WORKSPACE_PRESET_ERROR_CODES, ea as CAD_WORKSPACE_PRESET_SCHEMA, ta as CAD_WORKSPACE_PRESET_VERSION, D as CadActionButton, tt as CadAngleInput, mi as CadAnnotationScalePicker, Fi as CadBlockInsertOptions, Pi as CadBlockPalette, Ni as CadBlockTile, ki as CadColorPicker, Ai as CadColorPickerButton, rt as CadColorSwatch, Dr as CadCommandHistory, Ar as CadCommandLine, Or as CadCommandOptions, nn as CadCommandPrompt, gn as CadCompactWorkspaceRibbon, Xt as CadConfirmDialog, pi as CadConstraintBar, lt as CadContextMenuPopup, nt as CadCoordinateInput, He as CadCuiCommandPalette, Ve as CadCuiContextMenu, Ue as CadCuiCustomizer, Ne as CadCuiProvider, Be as CadCuiQuickAccess, ze as CadCuiRibbon, Xi as CadDataGrid, ee as CadDataRow, Yt as CadDialog, Sr as CadDockPanel, Cr as CadDockTabs, br as CadDocumentTabs, vr as CadDrawingSpaceTabs, ui as CadDynamicInput, j as CadEmptyState, Ii as CadFilterBar, fi as CadGripToolbar, O as CadIconButton, Ui as CadLayerPanel, Vi as CadLayerPicker, Hi as CadLayerRow, yr as CadLayoutTabs, ji as CadLinetypePicker, it as CadLinetypePreview, Mi as CadLineweightPicker, at as CadLineweightPreview, Ir as CadMeasureReadout, ct as CadMenu, Oi as CadMenuBar, st as CadMenuItem, ot as CadMenuSeparator, Jt as CadMovableOverlay, Cn as CadNavigationBar, $e as CadNumericInput, _i as CadObjectSnapMarker, di as CadObjectSnapMenu, Gi as CadObjectTree, vt as CadOverflowMenu, A as CadPanelFooter, w as CadPanelHeader, T as CadPanelSection, C as CadPanelShell, gi as CadPolarTracker, $t as CadPopover, Li as CadPropertyField, Bi as CadPropertyGrid, Ri as CadPropertyRow, zi as CadPropertySection, $i as CadQuickProperties, _t as CadRadialMenu, qi as CadReferenceList, E as CadSegmentTabs, Qi as CadSelectionCycler, Zi as CadSelectionFilter, vi as CadSelectionGrip, En as CadSelectionSetPanel, Fr as CadSelectionSummary, Ke as CadShortcutHint, tn as CadShortcutReference, Ye as CadSplitButton, Ti as CadSplitPane, k as CadStatGrid, Er as CadStatusBar, wr as CadStatusToggle, Di as CadSubmenu, Ki as CadTaskProgress, Zt as CadToast, Qt as CadToastStack, Je as CadToggleButton, qe as CadToolButton, Qe as CadToolPalette, Ze as CadToolbar, Xe as CadToolbarGroup, en as CadTooltip, Mr as CadUcsIndicator, et as CadUnitInput, jr as CadViewCube, hi as CadViewPresetPicker, Pr as CadViewportControls, Tn as CadViewportScalePicker, wn as CadVisualStylePicker, Ua as CadWorkspaceChromeControls, Zr as CadWorkspaceDockModeControl, ni as CadWorkspaceDockRail, Qr as CadWorkspaceDockResizeHandle, ri as CadWorkspaceDockZone, or as CadWorkspaceFocusToggle, nr as CadWorkspacePanelManager, rr as CadWorkspacePanelPreferences, Ia as CadWorkspacePresetManager, La as CadWorkspacePresetPanel, xr as CadWorkspaceProfileTabs, Pt as CadWorkspaceRibbon, _e as DEFAULT_CAD_CUI_SYSTEM, Xn as createCadWorkspacePanelPreferencesKey, wa as createCadWorkspacePreset, Ca as createCadWorkspacePresetSnapshot, fr as createCadWorkspaceProfile, ge as defineCadCuiSystem, Da as exportCadWorkspacePreset, Wn as getCadWorkspacePanelPreference, Na as getCadWorkspacePreset, Gn as groupCadWorkspacePanelsByDockZone, At as groupCadWorkspaceRibbonCommands, Oa as importCadWorkspacePreset, Pa as isCadWorkspacePresetNameTaken, Te as loadCadCuiState, B as matchesCadSelection, dr as nextCadWorkspaceLayoutName, R as normalizeCadSelection, z as normalizeCadSelectionRule, Ln as normalizeCadWorkspacePanelDockZone, In as normalizeCadWorkspacePanelPlacement, Un as normalizeCadWorkspacePanelPreferences, Vn as normalizeCadWorkspacePanels, Ta as normalizeCadWorkspacePreset, Ma as normalizeCadWorkspacePresets, ur as normalizeCadWorkspaceProfiles, mr as removeCadWorkspaceProfile, pr as renameCadWorkspaceProfile, Yn as resetCadWorkspacePanelPreferences, pn as resolveCadCompactWorkspaceRibbonGroups, xe as resolveCadCuiCommand, Se as resolveCadCuiCommandState, we as sanitizeCadCuiState, Ee as saveCadCuiState, Oe as selectCadCuiCommandGroups, De as selectCadCuiCommands, je as selectCadCuiRadialTree, pe as shouldHandleCadShortcut, Jn as updateCadWorkspacePanelPreference, Pe as useCadCui, Fe as useCadCuiCommand, Ie as useCadSelectionActions, Xr as useCadWorkspaceDock, ei as useCadWorkspaceDockRail, ar as useCadWorkspaceFocus, Zn as useCadWorkspacePanelPreferences, Ea as validateCadWorkspacePreset };
